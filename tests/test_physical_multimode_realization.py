...
def test_boundary_resolution_tradeoff_moves_cost_to_large_gravitating_mass() -> None:
    ...
    assert 9.0 < audit.resolved_radius_mass_equivalent_solar < 11.0
    assert 70.0 < audit.two_sided_mass_equivalent_earths < 80.0
...
def test_multimodes_do_not_reduce_fixed_geometry_energy_or_qi_control() -> None:
    ...
    assert math.isclose(
        audit.two_sided_negative_energy_magnitude_j,
        4.034185214e43,
        rel_tol=1e-9,
    )
