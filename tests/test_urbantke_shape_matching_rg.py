...
def test_common_orbit_closure_rejects_a_non_simple_reference() -> None:
    non_simple = geometric_self_dual_triple(np.eye(4))
    non_simple[0, 0] += 0.1

    assert float(np.min(np.linalg.eigvalsh(urbantke_metric_density(non_simple)))) > 0.0
    assert simplicity_residual(non_simple) > 1.0e-3
    with pytest.raises(ValueError, match="Plebanski-simple"):
        common_metric_block((non_simple, 2.0 * non_simple))


def test_common_orbit_closure_rejects_anti_self_dual_singleton() -> None:
    anti_self_dual = geometric_self_dual_triple(np.eye(4))
    anti_self_dual[:, 3:] *= -1.0

    assert simplicity_residual(anti_self_dual) < 1.0e-12
    with pytest.raises(ValueError, match="positive self-dual/orientation"):
        common_metric_block((anti_self_dual,))
