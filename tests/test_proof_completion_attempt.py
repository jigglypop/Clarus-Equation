...
def test_vcb_nlo_candidate_is_numerically_viable_but_conditional():
    vcb = by_name("|V_cb| NLO candidate")
    assert vcb.status == "conditional_pass"
    assert vcb.sigma_offset is not None
...
def test_vus_tree_fails_but_one_loop_candidate_passes():
    ...
    assert one_loop.status == "conditional_pass"
    assert one_loop.sigma_offset is not None
    assert abs(one_loop.sigma_offset) < 1


def test_as_raw_fails_but_readout_remains_conditional():
    raw = by_name("A_s raw")
    ...
    assert readout.status == "conditional_pass"
...
def test_ns_candidate_depends_on_transition_count():
    ...
    assert ns.status == "conditional_pass"
