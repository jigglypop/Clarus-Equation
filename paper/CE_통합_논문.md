...
Clarus Equation(CE)은 **상태·관계의 차원이 공간차원으로 나타나고, 같은 관계의 관측 방식에 따라 힘이 드러나며, 관측 밖의 잔여 성분이 시공간 곡률에 반응한다**는 구상을 연구한다. 본문의 공통식 유도는 §2.4와 §3.3–3.12에 둔다. 이미 계산한 세 채널 질량식은 이 출발 원리 전체를 대신하는 정의가 아니라 후반부의 조건부 구현이다. 최신 연구의 장별 분석과 판본 대조는 [재구성 근거](최신연구_분석과_논문구성.md)에 둔다.
...
이 절은 기존 순환 스펙트럼의 조건부 관측 결과다. §3.3–3.12의 관계 기하에서 해당 매개변수와 초기상태를 새로 도출해 얻은 점수는 아니다.
