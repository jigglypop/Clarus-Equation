# Clarus Equation 논문·원장 안내

`paper/`는 Clarus Equation의 통합 논문, 유도·강의와 검증 원장을 모은 정본 루트다. 이 안내는 처음 독자가 목적에 맞는 입구를 찾도록 돕는다. 독자는 수식·관측 비교·코드 검증이 서로 다른 지위를 가진다는 점에서 출발하며, 한 경로의 통과를 다른 경로의 증명으로 읽지 않는다.
...
CE의 최상위 목적과 모든 후보에 공통으로 적용할 성공 조건은 [연구 목표 계약](연구_목표_계약.md)에 고정한다. 암흑물질ㆍ암흑에너지는 가능한 거시적 readout이지, 연구가 미리 맞혀야 할 정체로 선결하지 않는다.
...
$Dq$ 조성과 전이 면 $1/D$를 결합하는 세부 우주론 branch의 목표는
...
최신 0차원 측정·접힘·암흑부문 연구는 큰 단일 보고서 대신 [최신 연구 조립 목차](6_최신_연구/00_읽기_지도.md)에서 장별로 읽는다. 그중 [하나의 typed 이력으로 읽는 0D--Plebanski 유한 증인](6_최신_연구/14_역방향_0D_Plebanski_조건부_완결.md)은 같은 shared tetrahedronㆍGram defectㆍreadoutㆍrecord가 이어지는 한 평탄 Lorentzian Plebanski/Einstein 유한 증인을 설명한다. 이어지는 [같은 이력의 비평탄 Plebanski 힌지](6_최신_연구/15_같은_이력의_비평탄_Plebanski_힌지.md)는 원래 두 simplex를 보존하고 하나를 더해 닫힌 hinge link 위의 constant-curvature 유한 증인을 설명한다. [proper vertex의 경계와 rank-one 수축](6_최신_연구/16_proper_vertex_경계와_rank_one_수축.md)은 flat 단일-vertex 외부 정리와 조건부 수축을 curved amplitude와 구분하고, [곡률 단일가지 가우스 템플릿](6_최신_연구/17_곡률_단일가지_가우스_템플릿.md)은 실제 curved block으로 가기 전의 local Gaussian 경계를 적는다. [분포적 refinement와 rigging map](6_최신_연구/18_분포적_refinement와_rigging_map.md)은 강한 Hilbert-norm 수렴을 쓰지 않는 직접계 모형을 분리하며, [선형화 Einstein 두 편광 수용 정리](6_최신_연구/19_선형화_Einstein_두_편광_수용_정리.md)는 supplied linearized action의 두 편광 기준을 적는다. [게이지 보존 Fierz--Pauli 격자 refinement](6_최신_연구/20_게이지_보존_Fierz_Pauli_격자_refinement.md) 뒤의 네 장은 두 미분 Fierz--Pauli 유일성, refinement Hessian/Ward와 Schur 유효커널, TT physical pole, 접힌 먼지와 상수 진공의 조건부 FLRW 전파를 차례로 분리한다. 이어지는 [future/outward normal과 $SL(2,\mathbb C)/SU(2)$ coset lift](6_최신_연구/28_future_outward_normal과_SL2C_coset_lift.md)는 고정 Lorentzian $1\to5$ 증인에서 future normal과 incidence별 outward sign을 분리하고 rotation-free lift를 구성한다. [접공간 $SU(2)$ section과 cell-local $J$-dual gluing](6_최신_연구/29_접공간_SU2_section과_cell_local_J_dual_gluing.md)은 improper Cholesky chart 반례 뒤에 right-handed coordinate section과 국소 phase convention을 붙이며, 이를 global Regge transport와 구별한다. [전역 flat coframe connection과 zero boost deficit](6_최신_연구/30_전역_flat_coframe_connection과_zero_deficit.md)은 declared common affine embedding에서만 성립하는 gauge-dependent global connection과 identity hinge loop를 설명한다. 이 문서들은 bare $0$차원 유도, standard multi-vertex amplitude, continuum GR, 암흑부문 확인을 주장하지 않는다. 작업공간 보고서는 과거 근거이고 이 목차와 연결된 `paper/` 문서가 독자용 최신본이다.

[exact oriented bivector와 full shape 반례](6_최신_연구/31_exact_oriented_bivector와_full_shape_반례.md)는 fixed flat 고전 bivector와 cross-cell transport를 분리하고, 이전 local section의 full labelled-shape 실패를 보인다. [full shape parity obstruction과 split Lorentz transport](6_최신_연구/32_full_shape_parity_obstruction과_split_Lorentz_transport.md)는 global pointwise-labelled $SU(2)$ repair의 반례와 flat Lorentz/Wigner 분리를 기록한다. [incidence spinor와 Cartan dual EH sector](6_최신_연구/33_incidence_spinor와_Cartan_dual_EH_sector.md)는 cell--tetrahedron incidence를 포함한 local spinor section, 유한 projector 부호 반례, Cartan-dual fixed-cell EH orientation/Plebanski sector candidate를 분리한다. [single-cell proper kernel contract](6_최신_연구/34_single_cell_proper_kernel_contract.md)는 한 cell의 Eq.-(53) target endpoint와 root-gauge-fixed relative frames를 고정하고, [$\mathbb{CP}^1$ pointwise proper coefficient](6_최신_연구/35_CP1_pointwise_proper_coefficient.md)는 그 frames에서 compact face pairing의 relative numerical coefficient를 평가한다. 이 결과들은 Haar integral, Regge phase, EH action/dynamics, global EPRL state나 amplitude를 주장하지 않는다.

### 9.1 먼저 읽기

처음 읽기는 전체 서사와 형식 지위 규약을 잡는 경로다. 세부 수식은 뒤 문서로 미루되, 공리와 미완성 다리를 건너뛰지 않는다.

0. [CE 진전 원장: 닫힌 것·죽은 경로·지금 단 하나의 표적](진전_원장.md) — 세션마다 갱신되는 한 화면 현황판. 처음 읽기와 모든 연구 세션의 시작점.
1. [CE 연구 목표 계약: 반복 잔차를 설명하는 공통 원리](연구_목표_계약.md)
2. [최신 연구 조립 목차: 측정, 접힘과 암흑 표현](6_최신_연구/00_읽기_지도.md)
3. [CE 통합 논문: 선택, 접힘, readout과 조건부 응용의 현재 지형](CE_통합_논문.md)
4. [선택과 접힘: 핵심 유도 사슬](5_유도/00_선택과_접힘.md)
5. [극장 개장과 서로 다른 좌석 무게의 에너지 장부](5_유도/08_극장과_좌석_에너지장부.md)
6. [양자 극장 개장: 다종 모드와 매끈한 질량 변화](5_유도/09_양자_극장_개장/00_읽기_순서.md)
7. [코어 독자 가이드](코어_독자_가이드.md)
8. [공리계와 기호](axium.md)
9. [경로적분과 조건부 장론](경로적분.md)
10. [상수·매개변수 원장](상수.md)
11. [핵심 정리 증명](검증_원장/참조_핵심_정리_증명.md)
...
[10장](6_최신_연구/10_플랑크_렌더링_경계와_0D_GR_브리지.md#달성한-조건부-구성--qd-m5-j2a부터-qd-m5-j2c까지)에,
