...
현재 `10_PreEq_LLM_manifest_verifier.md`의 count 기반 defect는 smoke-test용 저차 근사다. `paper/참조/4_공학적_활용/09_무차원_잔차장_환각억제.md`와 `paper/참조/9_등호이전/01_공리와증명.md` 기준으로는, 실제 verifier의 기본 단위는 답변 문자열이 아니라 **claim residual field**여야 한다.
