...
> **원장 범위.** 목적은 PMNS 구성의 조건부 수학과 관측 입력을 분리하는 것이다. 정의역은 명시한 유니터리 혼합 행렬과 질량·위상 매개변수이며, 입력은 외부 관측값 및 모델 선택, 출력은 조건부 행렬 관계다. 증명과 판본은 [핵심 정리 증명 원장](../../../검증_원장/참조_핵심_정리_증명.md) 및 우주론 판본·주장 원장 (미확보 경로: `../../검증_원장/상수_우주론_원장.md`)을 따른다. 독자 전제: 형식 지위와 외부 입력을 구분해 읽는 독자.
