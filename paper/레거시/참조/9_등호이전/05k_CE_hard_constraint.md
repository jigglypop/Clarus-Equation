...
이 문서는 hard feasible-set conditioning과 soft penalty Gibbs tilt를 구별하고, 유한 독립 모드에서 threshold 분율을 분석한다. 닫힌 것은 명시한 유한/가우시안 모형의 측도론이며, CE의 물리 constraint·penalty·온도 극한 순서는 설계 선택으로 남는다.

독자는 05i의 path prior와 05l의 uncertainty 경계를 먼저 읽는다. 조건부 측도, Brownian no-go, 유한모드 분율, 최소자 농축과 smooth tilt, 남은 물리 선택 순서로 읽는다.

## 0. 범위

hard constraint는 feasible set 위로 측도를 제한하고 soft constraint는 penalty를 지수 가중에 넣는다. 두 연산의 limit order와 정규화는 같지 않으며 이를 혼용하면 반례가 생긴다.
...
조건부 측도는 feasible set의 양의 기준 질량과 measurability를 요구한다. 영측도 constraint에서는 단순한 비율식 대신 regular conditional law나 근사 scheme이 필요하다.
...
Brownian prior와 kinetic regularity constraint는 서로 다른 path regularity에 놓일 수 있다. 다음 no-go는 hard conditioning의 분모가 0이 되는 대표적 반례를 분리한다.
...
유한 독립 Gaussian 모드는 threshold·분율·penalty를 명시적으로 계산할 수 있는 toy model이다. 이는 continuum CE mode spectrum이나 실제 noise law의 유도가 아니다.
...
고정 분율은 모드 수와 독립성·동일분포 가정 아래 정의된다. threshold의 단위와 정규화는 외부 설계 입력이다.
...
개수 분율과 action 가중 분율은 서로 다른 관측량이라 일반적으로 같은 결론을 내지 않는다. 어느 분율을 CE readout으로 채택할지는 모델 선택이다.
...
threshold event의 확률과 zero-temperature 최소자 농축은 다른 극한이다. penalty strength·inverse temperature·mode count의 순서를 고정하지 않으면 결과가 달라질 수 있다.
...
[../../검증_원장/등호이전_CE_supp_scaling_audit.md](../../../검증_원장/특화_감사/등호이전_CE_supp_scaling_audit.md) 4절에
있다.

## 5. Smooth tilt와 threshold 분율

smooth tilt는 hard indicator를 연속 penalty로 근사하는 설계다. convergence에는 penalty scale·정규화·uniform integrability가 필요하며 자동 등가는 아니다.
...
실제 CE constraint의 feasible set, noise ensemble, penalty scale, 측정 readout은 이 수학 모형이 정하지 않는다. 이 항목들은 경험 입력과 반증 조건을 가진 미완성 bridge다.
