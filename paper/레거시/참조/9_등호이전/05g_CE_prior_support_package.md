...
이 문서는 Gibbs 농축의 recovery mass가 prior support만으로 자동이 아님을 보이고, finite·atomic·full-support·tube recovery package를 구분한다. 닫힌 것은 확률측도와 topology 아래의 support 정리이며, CE 물리 prior 선택은 공리·외부 입력으로 남는다.

독자는 05e–05f의 good-rate와 topology package를 먼저 읽는다. recovery의 no-go, 충분조건, CE reweighting, full-support 구성과 권장 prior package를 순서대로 확인한다.

## 0. 목표

최소점 근방의 양의 prior 질량은 분모 하한의 핵심이며 full support라는 말만으로 항상 확보되지 않는다. 이 절은 정확한 regularity와 반례 경계를 고정한다.
...
support·near-minimum set·recovery mass의 표기는 선택한 topology와 Borel 구조에 상대적이다. 서로 다른 pathspace topology의 support를 혼용하지 않는다.
...
lower-semicontinuity와 full support는 열 근방의 질량을 말하지만 near-minimum set이 충분한 열린 근방을 포함하려면 최소점의 continuity 등이 더 필요하다. 다음 정의와 no-go가 그 공백을 분리한다.

### 정의 2.1: recovery mass

recovery mass는 각 에너지 허용오차에서 최소점 근방에 남는 양의 prior 질량을 요구한다. 이는 action의 성질이 아니라 prior와 topology를 함께 쓰는 조건이다.
...
no-go는 l.s.c.와 full support만으로 recovery가 따라오지 않는 반례를 제공한다. 따라서 이 두 조건만으로 CE prior를 승격할 수 없다.
...
다음 충분조건은 각기 다른 방식으로 recovery 질량을 보장한다. 이들은 상호 동치가 아니며 실제 CE prior가 어느 package인지 별도로 선택해야 한다.

### 정리 3.1: finite positive prior

유한 후보와 양의 prior에서는 최소 후보가 직접 양의 질량을 가진다. 이는 continuum prior의 support 결론으로 일반화되지 않는다.
...
최소점 atom은 가장 직접적인 recovery를 준다. atom 선택은 물리 prior의 자연스러움이나 uniqueness를 증명하지 않는다.
...
연속성은 near-minimum set 안에 열린 근방을 넣어 full support를 recovery로 바꾼다. l.s.c.만 있을 때는 앞 no-go가 적용될 수 있다.
...
tube recovery는 최소집합 주변의 양의 질량을 공리로 직접 요구한다. 이는 정리의 결과가 아니라 prior에 대한 채택 조건이다.
...
절대연속 재가중은 원 prior가 영질량인 영역에 새 support를 만들지 않는다. 따라서 CE factor가 missing recovery를 자동으로 복구한다는 해석은 반례에 취약하다.
...
separable metric pathspace에서는 dense atomic 구성으로 full-support 확률측도를 만들 수 있다. 구성의 존재는 그 prior가 CE 물리적으로 적절하다는 선택 근거가 아니다.

$\Gamma$가 separable metric space이면 full-support Borel probability는 항상 만들 수 있다.

### 정리 5.1: dense atomic full-support prior

정리는 dense set과 양의 합계 가중치를 사용한다. nonseparable 공간이나 다른 sigma-algebra에서는 추가 조건이 필요하다.
...
권장 package는 형식 recovery를 만족시키는 선택지를 비교한다. 이들은 data provenance·물리 대칭·실험 제약을 포함한 최종 prior 판정이 아니다.

CE 9_등호이전 문서군에서는 다음 package를 권장한다.

### Package F: finite mesh

finite mesh는 양의 discrete prior로 recovery를 직접 확보한다. continuum extrapolation은 별도 미완성이다.
...
continuous energy route는 topology와 minimizer continuity가 맞을 때 full support를 사용한다. l.s.c.만으로 충분하지 않다.
...
tube package는 필요한 질량 조건을 명시적으로 채택한다. 이는 물리 데이터에서 유도된 결과가 아니다.
...
atom package는 최소점 지식에 의존하므로 사후적으로 prior를 맞추는 위험을 분리해 보고해야 한다.
...
권장 공리는 good-rate 정리의 recovery 가정을 CE 문서에 명시하는 모델 선택이다. 공리 문구만으로 실제 prior sampling이나 support 검증이 완료되지는 않는다.
...
닫힌 support 정리와 물리 prior·pathspace·continuum 검증의 남은 부분을 분리한다. 표는 증거 등급을 올리는 절차가 아니다.
...
결론적으로 recovery mass는 prior·topology·energy regularity의 공동 조건이다. CE bridge는 이 조건을 실제 chosen prior에서 검증해야 한다.
