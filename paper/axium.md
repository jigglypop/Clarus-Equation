...
**[산출: R2 관측창 부분계]** 고정 계량의 $\pi=\delta T$는 gauge-dependent이므로 음의 $m_{\rm eff}^2$를 물리 mode의 질량으로 읽지 않는다. Einstein+clock 단일-clock 부분계에서 lapseㆍshift를 제거하면 $Q_s=XA/H^2>0$, $c_s^2=B/A>0$인 $\zeta$ 작용을 얻는다. 동결 관측창에서 $\min c_s^2=9.2138\times10^{-19}$, $\min Q_s/M_{\rm Pl}^2=3.3167\times10^5$와 $\dot\zeta$ 및 장파장 적분함수의 감소를 위한 기울기 $3.93909$, $3.469545$를 확인했다. 이는 single-clock 부분계의 필요조건 진단일 뿐, 암흑에너지 증명이나 전체 섭동 안정성 판정이 아니다.
...
| R2 단일-clock 관측창 필요조건과 $\dot\zeta$/적분함수 감소ㆍcutoff 여유 | **[조건부 산출]** |
