...
2026-09-09 최신 연구는 [주제별 읽기 지도](../../00_읽기_지도.md)로 재구성했다. 먼저 [기원 대화 분석](../../00_기원과_현재계획/01_기원_대화와_방향_진단.md)에서 현재 방향과 이전 검증기록의 경계를 확인한다.
...
CE의 최상위 목적과 모든 후보에 공통으로 적용할 성공 조건은 [연구 목표 계약](../../검증_원장/연구_목표_계약.md)에 고정한다. 암흑물질ㆍ암흑에너지는 가능한 거시적 readout이지, 연구가 미리 맞혀야 할 정체로 선결하지 않는다.
...
우주론 수치는 먼저 우주론 판본·주장 원장 (미확보 경로: `../../검증_원장/상수_우주론_원장.md`)에서
...
[공리계](../../00_기원과_현재계획/15_CE_공리계와_모형_사전.md)는 다음 작용을 하나의 일관된 EFT로 채택한다.
...
우주론 판본·주장 원장 (미확보 경로: `../../검증_원장/상수_우주론_원장.md`)에 별도로 둔다.
...
최신 0차원 측정·접힘·암흑부문 연구는 큰 단일 보고서 대신 [최신 연구 조립 목차](../../00_읽기_지도.md)에서 장별로 읽는다. 그중 [하나의 typed 이력으로 읽는 0D--Plebanski 유한 증인](../../03_블록_RG와_Plebanski/14_역방향_0D_Plebanski_조건부_완결.md)은 같은 shared tetrahedronㆍGram defectㆍreadoutㆍrecord가 이어지는 한 평탄 Lorentzian Plebanski/Einstein 유한 증인을 설명한다. 이어지는 [같은 이력의 비평탄 Plebanski 힌지](../../03_블록_RG와_Plebanski/15_같은_이력의_비평탄_Plebanski_힌지.md)는 원래 두 simplex를 보존하고 하나를 더해 닫힌 hinge link 위의 constant-curvature 유한 증인을 설명한다. [proper vertex의 경계와 rank-one 수축](../../03_블록_RG와_Plebanski/16_proper_vertex_경계와_rank_one_수축.md)은 flat 단일-vertex 외부 정리와 조건부 수축을 curved amplitude와 구분하고, [곡률 단일가지 가우스 템플릿](../../03_블록_RG와_Plebanski/17_곡률_단일가지_가우스_템플릿.md)은 실제 curved block으로 가기 전의 local Gaussian 경계를 적는다. [분포적 refinement와 rigging map](../../03_블록_RG와_Plebanski/18_분포적_refinement와_rigging_map.md)은 강한 Hilbert-norm 수렴을 쓰지 않는 직접계 모형을 분리하며, [선형화 Einstein 두 편광 수용 정리](../../04_중력_극한과_FLRW/19_선형화_Einstein_두_편광_수용_정리.md)는 supplied linearized action의 두 편광 기준을 적는다. [게이지 보존 Fierz--Pauli 격자 refinement](../../04_중력_극한과_FLRW/20_게이지_보존_Fierz_Pauli_격자_refinement.md) 뒤의 네 장은 두 미분 Fierz--Pauli 유일성, refinement Hessian/Ward와 Schur 유효커널, TT physical pole, 접힌 먼지와 상수 진공의 조건부 FLRW 전파를 차례로 분리한다. 이어지는 [future/outward normal과 $SL(2,\mathbb C)/SU(2)$ coset lift](../../05_Regge와_proper_kernel/28_future_outward_normal과_SL2C_coset_lift.md)는 고정 Lorentzian $1\to5$ 증인에서 future normal과 incidence별 outward sign을 분리하고 rotation-free lift를 구성한다. [접공간 $SU(2)$ section과 cell-local $J$-dual gluing](../../05_Regge와_proper_kernel/29_접공간_SU2_section과_cell_local_J_dual_gluing.md)은 improper Cholesky chart 반례 뒤에 right-handed coordinate section과 국소 phase convention을 붙이며, 이를 global Regge transport와 구별한다. [전역 flat coframe connection과 zero boost deficit](../../05_Regge와_proper_kernel/30_전역_flat_coframe_connection과_zero_deficit.md)은 declared common affine embedding에서만 성립하는 gauge-dependent global connection과 identity hinge loop를 설명한다. 이 문서들은 bare $0$차원 유도, standard multi-vertex amplitude, continuum GR, 암흑부문 확인을 주장하지 않는다. 작업공간 보고서는 과거 근거이고 이 목차와 연결된 `paper/` 문서가 독자용 최신본이다.

[exact oriented bivector와 full shape 반례](../../05_Regge와_proper_kernel/31_exact_oriented_bivector와_full_shape_반례.md)는 fixed flat 고전 bivector와 cross-cell transport를 분리하고, 이전 local section의 full labelled-shape 실패를 보인다. [full shape parity obstruction과 split Lorentz transport](../../05_Regge와_proper_kernel/32_full_shape_parity_obstruction과_split_Lorentz_transport.md)는 global pointwise-labelled $SU(2)$ repair의 반례와 flat Lorentz/Wigner 분리를 기록한다. [incidence spinor와 Cartan dual EH sector](../../05_Regge와_proper_kernel/33_incidence_spinor와_Cartan_dual_EH_sector.md)는 cell--tetrahedron incidence를 포함한 local spinor section, 유한 projector 부호 반례, Cartan-dual fixed-cell EH orientation/Plebanski sector candidate를 분리한다. [single-cell proper kernel contract](../../05_Regge와_proper_kernel/34_single_cell_proper_kernel_contract.md)는 한 cell의 Eq.-(53) target endpoint와 root-gauge-fixed relative frames를 고정하고, [$\mathbb{CP}^1$ pointwise proper coefficient](../../05_Regge와_proper_kernel/35_CP1_pointwise_proper_coefficient.md)는 그 frames에서 compact face pairing의 relative numerical coefficient를 평가한다. 이 결과들은 Haar integral, Regge phase, EH action/dynamics, global EPRL state나 amplitude를 주장하지 않는다.
...
1. [CE 연구 목표 계약: 반복 잔차를 설명하는 공통 원리](../../검증_원장/연구_목표_계약.md)
2. [최신 연구 조립 목차: 측정, 접힘과 암흑 표현](../../00_읽기_지도.md)
3. [CE 통합 논문: 관계 상태에서 공통 스펙트럼과 중력 반응으로](../../CE_통합_논문.md) — [연구 분석과 장 구성](../../00_기원과_현재계획/13_최신연구_분석과_논문구성.md)
...
8. [공리계와 기호](../../00_기원과_현재계획/15_CE_공리계와_모형_사전.md)
9. [경로적분과 조건부 장론](../../06_QFT_재설계/36A_조건부_장론과_확률_코어.md)
10. [상수·매개변수 원장](../../검증_원장/상수_조건부_산출_원장.md)
...
- [뇌 검증기준](../../검증_원장/특화_감사/뇌_검증기준.md)
- [검증 규약](../../검증_원장/경로적분_검증_규약.md)
- [형식 구조 원장](../../검증_원장/경로적분_전체_진리값_감사.md)
- [리만·등호이전 참조 안내](../리만과_등호이전.md) — 통합 논문의 본문과 분리한 보조 자료
- [형식적 수학 모델](../형식적_수학_모델과_증명.md)
...
[10장](../../02_인과기하와_우주론_연결/10_플랑크_렌더링_경계와_0D_GR_브리지.md#달성한-조건부-구성--qd-m5-j2a부터-qd-m5-j2c까지)에,
