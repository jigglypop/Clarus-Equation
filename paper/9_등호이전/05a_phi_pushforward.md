...
이 문서는 비선택 경로측도를 measurable kernel로 적분해 잔류장 readout $\phi$를 만드는 최소 측도론 도구를 고정한다. 수학적으로 닫히는 것은 finite measure와 integrable kernel의 pushforward이며, 이를 CE의 독립 물리장 $\Phi$로 읽는 것은 별도 공리·미완성 bridge다.

독자는 05장의 경로공간 package를 먼저 읽고, raw/conditional 잔류 측도, 유한 공식과 약수렴 안정성, 커널 후보 및 $\Phi$와의 구분 순서로 읽으면 된다. 모든 식은 measurable space와 적분가능성·정규화 조건을 전제로 한다.

## 0. 목표

05장의 간결한 적분식을 실제로 쓰려면 경로공간·선택집합·kernel의 정의역과 raw/conditional 질량 규약을 분리해야 한다. 이 절은 그 형식 지위를 먼저 밝히며 커널 선택을 물리적 유도로 바꾸지 않는다.
...
pushforward에는 후보 경로, 재가중 측도, 선택과 비선택의 measurable 분할, 출력 상태공간 및 kernel이 필요하다. 아래 기호는 입력 자료의 목록이며 CE 경로공간이나 $X$의 물리적 의미를 증명하지 않는다.
...
비선택 질량을 보존하는 raw 측도와 그 모양만 남기는 conditional 측도는 서로 다른 객체다. conditional 분모가 양수여야 하며, 어느 규약을 CE 에너지나 패턴으로 읽을지는 `[공리: 모델 선택]`이다.
...
bounded measurable kernel은 적분가능성을 보장하는 충분조건이므로 raw field를 형식적으로 정의한다. unbounded kernel에는 각 $\beta$에서의 $L^1$ 조건과 출력의 measurability를 별도로 확인해야 한다.
...
유한 경로공간에서는 pushforward가 가중 유한합으로 환원되어 raw와 conditional의 차이가 분모에 드러난다. 이 계산은 continuum limit나 수치 sampling 오차를 통제하지 않는다.
...
측도 극한을 field readout으로 옮기려면 kernel이 약수렴의 test function으로 허용되어야 한다. 다음 정리는 compact metric space와 bounded continuous kernel의 충분조건이며, pointwise measurable kernel만으로는 성립하지 않는다.
...
어떤 kernel이 경로 정보를 잔류장으로 읽을지는 수학이 자동으로 결정하지 않는다. 표의 후보는 가능한 구성일 뿐, locality·covariance·단위·action 및 관측 검증 없이 CE의 물리 $\Phi$라고 부를 수 없다.
...
소문자 readout과 기존 CE의 대문자 유효 자유도는 정의역과 역할이 달라 자동으로 같지 않다. 아래 가능한 동일시는 kernel을 추가로 채택한 조건부 구성이고, stress tensor나 Euler–Lagrange 방정식의 유도는 제공하지 않는다.
...
물리 bridge에는 측도론 정의 외에도 경로 topology·선택집합 measurability·kernel 정규화·작용과 관측 연산자가 필요하다. 표는 필요한 입력을 점검하는 명세이지 동일시가 완료되었다는 증거가 아니다.
...
결론적으로 적분의 존재는 비교적 약한 측도론 가정으로 닫히지만, 무엇을 kernel으로 선택해 어떤 물리량으로 읽을지는 남는다. 이 선택의 반례·적용 범위는 05장의 action/prior 검증과 함께만 평가할 수 있다.
