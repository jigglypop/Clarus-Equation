...
가 된다. 이것이 C4-REGGE-ONE-TO-FIVE-BOUNDARY-SCHUR-23B의 **조건부 고전 boundary Schur identity**다. 재현 스크립트 (과거 검산: `verify/Q-0019/F-01/regge_one_to_five_boundary_hessian.py`)의 과거 대응 테스트 기록은 6 passed, Regge 묶음은 15 passed다. 대응 테스트 파일은 현재 체크아웃에 없으므로 이 과거 숫자를 현행 green evidence로 사용하지 않는다. 실행 성공도 식 (7)의 증명 수가 아니라 구현 재현이다.
