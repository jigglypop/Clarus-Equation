...
- 스킬·에이전트·TOML·프롬프트·하네스는 모두 **목표 계약**, **완료 조건**,
  **목표 이탈**, **복귀 행동**을 요구한다.
- 계획은 사용자에게 단계 순서와 각 단계가 남길 증거를 설명하며, 시작·단계
  전환·반례·상태 요약마다 `[정렬]`/`[이탈]`/`[범위변경]`을 판정한다.
- 목표와 연결되지 않는 단계는 복귀 행동으로 정렬을 회복하기 전까지 다음
  실행 담당에게 넘기지 않는다.
- 하네스 검사는 위 다섯 계약 진입점이 빠졌을 때 실패한다.
