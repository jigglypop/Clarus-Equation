...
``J^pi=3/2+`` entrance channel gives

    sigma_pol(E) / sigma_unpol(E) = 1 + P_D P_T / 2.

The factor is energy independent in that approximation, so it commutes with
the Maxwellian integral.  Ab-initio NCSMC calculations support the underlying
channel dominance, but a reactor-rate polarized D--T source and an in-plasma
polarization-lifetime measurement have not yet been demonstrated.  The
conditional ideal-projector benchmark is therefore kept separate from the
physical reaction and engineering gates.
"""
...
class PolarizedFuelPumpLedgerAudit:
    ...
    source_throughput_declared: bool
    measured_wall_plug_energy_per_polarized_dt_pair_ev: float | None
    measured_wall_plug_energy_per_polarized_dt_pair_available: bool
    measured_pair_energy_below_break_even: bool
...
def audit_polarized_fuel_pump_ledger(
    *,
    ...
    measured_wall_plug_energy_per_polarized_dt_pair_ev: Real | None = None,
    ...
    if measured_wall_plug_energy_per_polarized_dt_pair_ev is None:
        ...
        measured_pair_energy_ev = _finite_real(
            ...
            name="measured_wall_plug_energy_per_polarized_dt_pair_ev",
        )
        if measured_pair_energy_ev < 0.0:
            raise ValueError(
                "measured_wall_plug_energy_per_polarized_dt_pair_ev must be nonnegative"
            )
    ...
    return PolarizedFuelPumpLedgerAudit(
        ...
        engineering_margin_energy_per_injected_pair_ev=(electrical_break_even_ev / margin),
        ...
        source_throughput_declared=True,
        measured_wall_plug_energy_per_polarized_dt_pair_ev=measured_pair_energy_ev,
        measured_wall_plug_energy_per_polarized_dt_pair_available=(measured_pair_energy_available),
        measured_pair_energy_below_break_even=measured_pair_energy_below_break_even,
...
def _pump_numeric_integrity(
    ...
    *,
    ...
    measured = pump.measured_wall_plug_energy_per_polarized_dt_pair_ev
    numeric_values = (
    ...
    return all(
        (
            ...
            pump.engineering_margin_energy_per_injected_pair_ev
            == electrical_break_even_ev / margin,
...
def physical_spin_branch_gate_pass(
    *,
    ...
    """Require every reaction, source, retention, and wall-plug leaf gate."""
    ...
    if not (
        ...
        and _retention_numeric_integrity(retention, expected_target=target)
    ...
    required_true_flags = (
        ...
        pump_ledger.source_throughput_declared,
        pump_ledger.fixed_exposure_burn_probability_model_evaluated,
        pump_ledger.conservative_fixed_exposure_budget_used,
        pump_ledger.measured_wall_plug_energy_per_polarized_dt_pair_available,
        pump_ledger.measured_pair_energy_below_break_even,
...
def current_fusion_spin_polarization_control_report() -> FusionSpinPolarizationControlReport:
    ...
    return FusionSpinPolarizationControlReport(
        schema_version="fusion-spin-polarization-control-v5",
        target_reaction=target_reaction,
        source_design_reaction=source_design_reaction,
        retention=retention,
        ...
        next_required_gate=(
            "MEASURED_POLARIZED_TRITIUM_SOURCE_WALL_PLUG_LEDGER_PLUS_"
            "DT_PLASMA_RETENTION_AT_DECLARED_THROUGHPUT"
        ),
        conclusion=(
            "Spin polarization clears the non-CE ideal-quartet control only. No "
            "physical engineering or CE branch is promoted without an energy-dependent "
            "polarized rate, Czz validation, tritium source, retention, and wall-plug ledger."
        ),
    )
