...
$$
m_{\text{low}} \sim \frac{\hbar c}{\lambda_{\text{gal}}} \sim \frac{197\ \text{MeV·fm}}{10\ \text{kpc}} \sim 10^{-20}\ \text{MeV} \sim 10\ \text{eV}
$$

이런 초경량 모드는 큐비트 환경에서 TLS(Two-Level System)와 유사한 저주파 코히런트 노이즈로 나타날 수 있다. 본 장에서는 이 저에너지 모드가 QEC와 **양립 가능한지, 아니면 QEC를 심각하게 방해하는지**를 수치 시뮬레이션으로 검증한다.
...
`sfe_core` QEC 엔진 (`qec.rs`)은 다음 조건에서 반복 코드(Repetition Code) QEC를 시뮬레이션한다.
...
즉, QEC가 물리 오류율보다 논리 오류율을 오히려 약간 더 키우는 **임계값 바로 아래 레짐**에 있다. 이는 distance=5, noise=0.10 조건이 QEC 임계 근방의 까다로운 영역임을 의미한다.
