...
impl EffectiveMassCalculator {
    ...
    /// 입자가 클라루스장(R)을 통과하며 잃어버리는 유효 질량/에너지 계산
...
pub fn run_muon_specificity_analysis() {
    ...
    println!("  → 클라루스 보손 교환이 10^7배 더 강하게 작용");
    ...
    println!("\n  → 질량-비례 결합으로 뮤온이 클라루스장을 207배 강하게 느낌");
    ...
    println!("     → 무거운 입자일수록 클라루스장과 강하게 결합");
    ...
    println!("\n  3. 결합 효과: ΔE ∝ m_ℓ^4");
    println!("     → 뮤온에서 10^7배 강한 클라루스 보손 효과");
