...
2. `.claude\hooks\python.cmd python .claude\hooks\lib\ledger.py summary`로 active 질문·최근 항목·escalated를 본다.
3. `git status --short`와 `git log -5 --format='%h %ad %s' --date=short`로 미커밋 변경과 최근 작업을 확인한다.
4. 기계 상태와 이론 지위를 분리해 보고하고, §2 표적에 대한 다음 최소 작업을 한 줄로 제안한다. 진전 원장 갱신일이 마지막 커밋보다 오래됐으면 첫 줄에 적는다.
