...
`reality_stone` 제거 후 저장소 전체의 무기록 Python 구문 검사는 `.claude/hooks/python.cmd source`를 사용한다. 이 모드는 `.codex`, `tests`, `examples`, `experiments`, `paper`의 `.py`를 메모리에서 AST 파싱하며 pyc나 cache를 만들지 않는다. focused pytest가 제거된 모듈을 요구하면 환경 실패로 기록하고 독립 실행 가능한 가장 가까운 테스트를 선택한다.
