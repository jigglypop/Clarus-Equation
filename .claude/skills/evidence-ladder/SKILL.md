...
description: 증거 등급 L0~L4를 매기거나 읽을 때, 어떤 원장 항목을 논문(paper/)에 인용해도 되는지 판단할 때, 원장 항목의 level 필드를 채울 때 반드시 참조. judge와 paper-writer가 쓴다.
---
...
- sympy가 설치되지 않은 환경에서는 `symbolic`이 항상 skipped이므로 최고 등급이
  L2다. 이 경우 L3 승격은 관리자가 sympy를 설치한 뒤 같은 derivation을 재검증해야
  한다. 이유: 기계검증 없는 L3는 사다리 정의 위반이다.
...
이유: 나중에 등급이 바뀌면 추적해서 고쳐야 한다.

실제 예시는 `references/examples.md`.
