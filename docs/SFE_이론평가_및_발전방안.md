...
#### 3.6 (해법) 최소 확장안과 우선순위
병목을 줄이되 과적합을 피하기 위해, 1-파라미터 최소 확장부터 검증한다.
...
#### 3.7 (핵심) 5상수 무피팅 해법 후보
위 3.6의 수치 최적화는 "탐색 단계"였고, SFE의 본래 목적(5상수 기반 무피팅)과 엄밀히 일치하지 않는다.  
목표 정합을 위해 다음처럼 **상수-유도형(무피팅)**으로 재정의한다.

**후보 A: 부트스트랩 누적량 보정(무피팅)**
$$\ln\varepsilon^2=-(1-\varepsilon^2)D_{\text{eff}}+\frac{\kappa_2}{2},
\qquad \kappa_2 \equiv \alpha_s\,\delta$$
- 수치: $\alpha_s\delta=0.02096$ (탐색 최적 0.022에 근접)
- 결과: $\chi^2_{\mathrm{red}}=1.28 \to 0.61$

**후보 B: 암흑분할 NLO 보정(무피팅)**
$$\frac{\Omega_{DM}}{\Omega_\Lambda}
=\alpha_sD_{\text{eff}}\!\left[1+\xi\,\alpha_sD_{\text{eff}}+O(\alpha_s^2)\right],
...
- 정리된 닫힘식 기준 우주론 다중 데이터셋에서 $\chi^2_{\mathrm{red}}\approx1.28$ (무피팅)
...
- Planck계열에서 $\Omega_b$ 저편향이 총 오차의 주 병목(관측량 기준 55.5%)
