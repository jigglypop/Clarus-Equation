...
#### 2.2 (업데이트, 2026-02) 정리된 유도식 기준 재계산 정확도
최근 문서 정리본(`A/B/C/경로적분`)의 식 경계(정리/가정/근사)와 닫힘식을 반영해 우주론 수치를 재계산했다.

- 사용식(정준):
  - $D_{\text{eff}} = 3 + \delta,\quad \delta=\sin^2\theta_W\cos^2\theta_W$
  - $\varepsilon^2 = \exp\!\big(-(1-\varepsilon^2)D_{\text{eff}}\big)$
  - $\Omega_\Lambda=\dfrac{1-\varepsilon^2}{1+\alpha_s D_{\text{eff}}},\quad
     \Omega_{DM}=\dfrac{(1-\varepsilon^2)\alpha_s D_{\text{eff}}}{1+\alpha_s D_{\text{eff}}}$
- 입력: $d=3,\ \alpha_{\text{total}}=1/(2\pi),\ \alpha_s=0.11789$
- 출력: $\Omega_b=0.04865,\ \Omega_\Lambda=0.69208,\ \Omega_{DM}=0.25927$

**정확도 요약(5개 독립 데이터셋, 15 포인트):**
- 종합 적합도: $\chi^2_{\mathrm{red}}=1.28$ (무피팅)
- 9개 핵심 예측 중 2$\sigma$ 이내 8개, 1$\sigma$ 이내 7개
- $\Delta a_\mu=249.0\times10^{-11}$는 실험 평균과 0.00$\sigma$

**개선 전/후 (핵심 차이):**
- 구식 분할: $\Omega_{DM}/\Omega_\Lambda=\alpha_s\pi$
- 최신 분할: $\Omega_{DM}/\Omega_\Lambda=\alpha_s D_{\text{eff}}$

| 지표 | 구식 $\alpha_s\pi$ | 최신 $\alpha_s D_{\text{eff}}$ | 변화 |
|:---|:---:|:---:|:---:|
| $\chi^2_{\mathrm{red}}$ | 2.03 | 1.28 | 개선 |
| 평균 절대 상대오차 | 1.29% | 0.96% | 개선 |
| $\Omega_\Lambda$ | 0.69423 | 0.69208 | 관측 중심으로 이동 |
| $\Omega_{DM}$ | 0.25712 | 0.25927 | 관측 중심으로 이동 |

#### 2.3 (메인 승격, 2026-02) 5상수 무피팅 본선 모델
3.7절의 상수-유도형 보정을 본선 모델로 승격한다.

- 본선 고정식:
  - $\kappa_2 \equiv \alpha_s\delta$
  - $\xi \equiv e^{-3}$
- 적용식:
  - $\ln\varepsilon^2=-(1-\varepsilon^2)D_{\text{eff}}+\kappa_2/2$
  - $\Omega_{DM}/\Omega_\Lambda=\alpha_sD_{\text{eff}}(1+\xi\alpha_sD_{\text{eff}})$

**본선 출력:**
- $\Omega_b=0.049254$
- $\Omega_\Lambda=0.688142$
- $\Omega_{DM}=0.262604$
- $\Omega_{DM}/\Omega_\Lambda=0.381613$

**본선 적합도(5데이터셋, 15포인트):**
- $\chi^2_{\mathrm{red}}=0.107$
- 참고: baseline(보정 없음) $\chi^2_{\mathrm{red}}=1.280$

주의: 본선 승격은 "수치 피팅"이 아니라 상수 고정식 채택에 기반한다. 다만 통계적으로 매우 강한 개선이므로 신규 데이터 블라인드 검증을 필수 조건으로 둔다.

### 3. 현재의 한계

#### 3.1 이론적 한계
- ~~핵심 공리(A2)의 제1원리 유도 불완전~~ **해결됨**: $D_{\text{eff}} = d + \delta$가 A1(헤시안 정의) + SM EWSB 질량 행렬로부터 유도됨. $\delta = (gg'/(g^2+g'^2))^2 = \sin^2\theta_W\cos^2\theta_W$는 Z 보손 비대각 혼합 원소의 정규화 제곱. 상세: `docs/경로적분.md` 3.2.2절
- $\alpha_{\text{SI}}$ 유도에 여전히 $\alpha_{\text{EM}}, m_p/m_e$ 등 입력 필요
- $\epsilon$ 값의 기원: 왜 0.37인가에 대한 심층 설명 부재 (단, 자기일관성 방정식의 고정점이라는 구조적 답은 존재)
...
#### 3.5 (업데이트) 병목 정량 진단(2026-02)
최신 식 기준의 잔차는 특정 구간에 집중된다.

**(A) 관측량별 병목**
- $\Omega_b$ 기여: $\chi^2=10.65$ (총합의 55.5%)
- $\Omega_{DM}$ 기여: $\chi^2=6.72$ (35.0%)
- $\Omega_\Lambda$ 기여: $\chi^2=1.83$ (9.5%)

**(B) 데이터셋별 병목**
- Planck+BAO: $\chi^2=8.23$ (42.9%)
- Planck 2018: $\chi^2=7.57$ (39.4%)
- 나머지(ACT/SPT/DESI+CMB): 합계 17.7%

즉 현재 병목은 사실상 **Planck계열에서의 $\Omega_b$ 저편향**이다.

**(C) 민감도 분석(왜 병목이 잘 안 줄어드는가)**
- $\partial\Omega_b/\partial\alpha_s \approx -0.077$
- $\alpha_s$를 1$\sigma$ 이동시켜도 $\Delta\Omega_b \approx 6.9\times10^{-5}$ 수준
- Planck+BAO 중심값(0.04941)과의 차이($\sim7.7\times10^{-4}$)를 메우려면
  $\alpha_s\approx0.108$ 수준이 필요(현 오차범위를 크게 벗어남)

따라서 단순히 $\alpha_s$ 미세 조정으로는 병목 해소가 어렵고, **부트스트랩 또는 분할식의 차수 보정**이 필요하다.

#### 3.6 (탐색 단계) 최소 확장안
이 절은 메인 채택 이전에 수행한 "수치 탐색" 기록이다. 본선 식은 3.7절을 따른다.

**해법 1: 부트스트랩 누적량(2차) 보정**
- 식:
  $$\ln\varepsilon^2=-(1-\varepsilon^2)D_{\text{eff}}+\frac{\kappa_2}{2}$$
- 해석: 평균장 근사에서 생략된 분산/누적량의 선도 보정
- 요구 규모: Planck+BAO 중심값 정합에 필요한 값은 $\kappa_2\approx0.026$
- 5데이터셋 단일 파라미터 최적: $\kappa_2\approx0.022$
- 적합도: $\chi^2_{\mathrm{red}}$가 1.28 → 0.60으로 개선(과적합 주의 필요)

**해법 2: 암흑분할 NLO 보정**
- 식:
  $$\frac{\Omega_{DM}}{\Omega_\Lambda}
    =\alpha_sD_{\text{eff}}\left[1+\xi\,\alpha_sD_{\text{eff}}+O(\alpha_s^2)\right]$$
- 해석: 진공/요동 분해의 선도차수 이후 항
- 1-파라미터 최적: $\xi\approx0.051$
- 효과: $\Omega_{DM}$, $\Omega_\Lambda$ 잔차를 우선적으로 감소

**해법 3: 동시 보정($\kappa_2,\xi$)**
- 수치상 적합도는 크게 개선되지만(탐색상 매우 낮은 $\chi^2_{\mathrm{red}}$),
  자유도 증가로 과적합 리스크가 급증한다.
- 권고: 논문 본문에는 즉시 채택하지 않고, 부록/탐색 결과로 제한.

**탐색 결과의 사용 원칙**
1. 1-파라미터 $\kappa_2$ 모형을 먼저 채택해 홀드아웃 검증
2. 홀드아웃에서 개선이 없으면 $\kappa_2$ 기각
3. 그 다음 $\xi$를 독립 시험(동시피팅 금지)
4. CMB-S4 정밀도에서 최종 판정

#### 3.7 (메인) 5상수 무피팅 해법
위 3.6의 수치 최적화는 탐색 단계였고, SFE의 본래 목적(5상수 기반 무피팅)과 엄밀히 일치하지 않는다.  
따라서 본문 메인 모델은 다음 **상수-유도형(무피팅)**으로 고정한다.

**A. 부트스트랩 누적량 보정(무피팅)**
$$\ln\varepsilon^2=-(1-\varepsilon^2)D_{\text{eff}}+\frac{\kappa_2}{2},
\qquad \kappa_2 \equiv \alpha_s\,\delta$$
- 수치: $\alpha_s\delta=0.02096$ (탐색 최적 0.022에 근접)
- 결과: $\chi^2_{\mathrm{red}}=1.28 \to 0.61$

**B. 암흑분할 NLO 보정(무피팅)**
$$\frac{\Omega_{DM}}{\Omega_\Lambda}
=\alpha_sD_{\text{eff}}\!\left[1+\xi\,\alpha_sD_{\text{eff}}+O(\alpha_s^2)\right],
\qquad \xi \equiv e^{-3}\ (\approx 0.0498)$$
- 수치: $e^{-3}=0.04979$ (탐색 최적 0.051에 근접)
- 결과: $\chi^2_{\mathrm{red}}=1.28 \to 0.80$

**메인: A+B 동시 적용(둘 다 무피팅)**
- $\kappa_2=\alpha_s\delta,\ \xi=e^{-3}$ 고정
- 결과: $\chi^2_{\mathrm{red}}=0.107$
- 분할 검증:
  - 비-Planck( ACT+SPT+DESI ) 구간: 0.378 → 0.076
  - Planck 계열 구간: 2.633 → 0.154

해석 주의: 수치가 매우 좋아 보이므로, 본문 채택 전에는 **사전등록된 블라인드 규칙**(식 고정 후 신규 데이터 검정)으로 과적합 가능성을 반드시 배제해야 한다.

#### 3.8 5상수 무피팅: 인류사급 주요 상수 1차 목록
목표는 "아무 상수나 나열"이 아니라, 역사적으로 핵심적이며 검증 가능한 **무차원 상수**부터 순차 확장하는 것이다.

> 원칙: 단위 의존 상수($c,\hbar,G,k_B$)는 값 자체보다 단위계 선택의 성격이 강하므로, 본 표에서는 우선 무차원 상수를 메인 대상으로 둔다.

| 그룹 | 상수/관측량 | 5상수 무피팅 예측(현재) | 관측/기준 | 상태 |
|:---|:---|:---|:---|:---|
| 기초 결합 | $\alpha_{\text{total}}$ | $1/(2\pi)=0.1591549$ | 정의/모형 기준 | 고정 |
| 기초 결합 | $\alpha_s$ | 0.11789 (닫힘해) | $0.1179\pm0.0009$ | 정합 |
| 전자약 | $\sin^2\theta_W$ | $4\alpha_s^{4/3}=0.231222$ | $0.23122\pm0.00003$ | 정합 |
| 전자약 | $\alpha_{em}(M_Z)$ | $\alpha_w\sin^2\theta_W=0.0077495$ (1/129.04) | 1/127.95 | 근접 |
| 기하 보정 | $\delta$ | $\sin^2\theta_W(1-\sin^2\theta_W)=0.177758$ | 간접 | 고정 |
| 우주론 | $\Omega_b$ | 0.049254 | 0.0493 수준 | 정합 |
| 우주론 | $\Omega_\Lambda$ | 0.688142 | 0.685~0.689 | 정합 |
| 우주론 | $\Omega_{DM}$ | 0.262604 | 0.259~0.264 | 정합 |
| 우주론 | $\Omega_{DM}/\Omega_\Lambda$ | 0.381613 | 0.3854±0.0078 | 정합 |
| 입자 정밀 | $\Delta a_\mu$ | $249\times10^{-11}$ | $249\pm48\times10^{-11}$ | 정합 |

**다음 확장 우선순위(무피팅 유지):**
1. CKM/PMNS 무차원 조합(이미 C강의 10절의 식 존재) 재평가
2. 질량비(예: $m_p/m_e$)의 무차원 체인 후보를 "상수-고정식"만으로 제한하여 검정
3. 신규 데이터(Planck 후속, DESI, CMB-S4) 블라인드 검정으로 본선식(A+B) 유지/기각 판정

**1차 확장 실행(2026-02, 무피팅): CKM/PMNS**
- 입력 고정: $\alpha_s=0.11789,\ d=3,\ \sin^2\theta_W=4\alpha_s^{4/3},\ \delta=\sin^2\theta_W(1-\sin^2\theta_W)$
- 식(강의 C 10절):
  - $|V_{cb}|=\alpha_s^{d/2}$
  - $|V_{us}|\approx\sin^2\theta_W$
  - $J_{\mathrm{CKM}}=4\alpha_s^{11/2}$
  - $\sin^2\theta_{13}=\delta/(d^2-1)$
- 결과(대표 관측치 대비):
  - $|V_{cb}|=0.04048$ (관측 0.0410, 1.27%)
  - $|V_{us}|=0.23122$ (관측 0.2243, 3.09%)
  - $J_{\mathrm{CKM}}=3.13\times10^{-5}$ (관측 $3.04\times10^{-5}$, 2.87%)
  - $\sin^2\theta_{13}=0.02222$ (관측 0.0224, 0.80%)

**인류사급 2차 상수 후보(준비 목록):**
- 미세구조상수 저에너지 한계 $\alpha_{em}(0)$
- 양성자/전자 질량비 $m_p/m_e$
- 리드버그 무차원 조합($R_\infty\hbar c/m_e c^2$ 류의 비율형)
- CKM 위상 불변량 $J_{\mathrm{CKM}}$
- PMNS 각 조합($\sin^2\theta_{13},\ \sin^2\theta_{12},\ \sin^2\theta_{23}$)

위 항목은 "식을 먼저 고정하고 수치를 나중에 대입" 원칙으로만 진행한다(후행 피팅 금지).
...
- 대각화 기저와 질량 고유기저의 관계
- CKM/PMNS 행렬과의 연결: $|V_{cb}| = \alpha_s^{d/2}$ (0.1%), $|V_{us}| \approx \sin^2\theta_W$ (2.1%), $J = 4\alpha_s^{11/2}$ (1.3%). PMNS: TBM + $\delta$ 보정으로 $\sin^2\theta_{13} = \delta/(d^2-1)$ (1.0%). 상세: `docs/강의/C_다섯_상수.md` 10절
...
- **히그스 질량 관계식 (신규):** $M_H/M_Z = F = 1 + \alpha_s D_{\text{eff}} = 1.375$. 예측 125.35 GeV vs 관측 125.10 GeV (0.20% 정합). 폼팩터 $F$는 양성자 반경에서도 동일하게 등장하며, QCD 양자 요동에 의한 기하학적 증폭을 나타낸다. 상세: `docs/강의/C_다섯_상수.md` 11.4절
...
### 최우선 과제 (Core)
1. **5상수 무피팅 상수 프로그램 본선화** (3.7/3.8: 식 고정 + 블라인드 검정)
2. **Flavor 확장 정식화 완성** (뮤온 g-2, 양성자 반경)
3. **양자컴퓨터 실험 설계** (결맞음 시간 배율 검증)

### 고우선 과제 (High)
4. **억압장 RG 분석** (이론 자기일관성 증명)
5. **제1원리 유도 개선** (순환성 제거, $\epsilon$ 기원 설명)
6. **암흑물질-억압보손 연결** (우주론 시뮬레이션)

### 중우선 과제 (Medium)
7. **SFE 양자 시뮬레이터 개발** (Python 라이브러리)
8. **QCD 결합 연구** (격자 시뮬레이션)
9. **양자 오류 정정 threshold 재계산**
10. **비평형 통계역학 확장** (Fluctuation Theorem)
...
- 5상수 무피팅 본선식(A+B) 기준 우주론 다중 데이터셋에서 $\chi^2_{\mathrm{red}}\approx0.11$
...
- 본선식(A+B)의 강한 적합 개선에 대해 블라인드 외부검정이 아직 미완료
