...
즉 "$0.37$"과 "$0.04865$"는 서로 다른 대상(단위 상수와 최종 생존율)이며, 별도 이론 버전이 아니다.
...
CE의 세 공리가 생존함수 $S(D)$에 부과하는 조건은 다음과 같다.
...
**유도.** 상태 $|\psi\rangle = \sum_k c_k |k\rangle$에서 접힘이 분지를 분리한다. 각 분지 $k$의 가중치 $W_k$에 대해서는 세 조건이 성립한다.

1. **가산성(B1에서).** 독립 분지의 가중치는 가산적이다. 즉 $W(A \cup B) = W(A) + W(B)$이다. 이는 접힘의 독립성(B1)의 직접적 결과다.
...
$1$은 **완전의 기준선**이다. 접힘이 없을 때($d = 0$)의 상태를 정의하고, 부트스트랩의 수렴을 보장한다. 모든 접힘은 $1$로부터의 이탈이며, 모든 물리는 $1$이 아닌 것에서 발생한다.
...
이 다섯 상수는 독립이 아니다. 오일러 항등식이 이를 보여 준다.
...
**(i)** 경로적분에서 클라루스장 $\Phi$에 의한 생존 확률을 $P_{\text{survive}} = \langle e^{-\Phi} \rangle$로 정의한다.
...
억압된 에너지 $\Omega_\Phi = 1 - \varepsilon^2$는 두 성분으로 분리된다.
...
중성미자 질량은 다음 요소들이 결합해 복사적으로 생성된다.

- 힉스 포탈을 통한 2-loop Weinberg 연산자
- Majorana 위상 통과의 1-loop 위상공간
- DE 분율
- SU(d) Casimir 멱 분배(외부 ν chiral flip + 내부 lepton dominance)
- Weinberg op anomalous-dim NLO RG running

$$
m_{\nu_l}
= \frac{\delta^4 \,(1 - \alpha_s/\pi)}
{(16\pi^2)^2 \cdot 32\pi^3 \cdot (1+R)}
\cdot m_l^{(d+2)/(d^2-1)}
\cdot m_\tau^{(d^2-d-3)/(d^2-1)}
$$

$d = 3$을 대입하면 $m_l^{5/8} \cdot m_\tau^{3/8}$가 된다. 멱 합은 $1$이므로 질량 1제곱이 보존된다. 또한 $d^2-1 = 8$은 SU(3) adjoint Casimir이며, $|V_{ub}|$ 도출과 같은 기하를 쓴다.

각 인자의 역할은 다음과 같다.

- $\delta^4 = \lambda_{\text{HP}}^2$: $\pi$와 $0$에서 오는 $\sin^2\theta_W \cos^2\theta_W$ 구조
- $(16\pi^2)^2$: 2-loop 인자
- $32\pi^3 = 2\pi \cdot 16\pi^2$: Majorana 위상 1-loop 위상공간
- $1/(1+R)$, $R = 0.38063$: DE 분율
- $(1 - \alpha_s/\pi) = 0.9625$: Weinberg dim-5 연산자의 anomalous-dim NLO 보편 보정(SM Wilson 계수 leading-log 형식)
- $m_l^{5/8}$: 외부 ν chiral flip
- $m_\tau^{3/8}$: 내부 lepton sum의 무거운 lepton dominance(CKM W-loop top dominance와 동형)

| 세대 | $m_l$ (MeV) | $m_\nu$ (meV) | 관측 (meV) | 잔차 |
|---|---|---|---|---|
| $e$ → $\nu_1$ | 0.511 | **0.306** | ≲ 1 | 양립 |
| $\mu$ → $\nu_2$ | 105.66 | **8.566** | 8.66 | **−1.09%** |
| $\tau$ → $\nu_3$ | 1776.86 | **49.986** | 50.5 | **−1.02%** |

$\sum m_\nu = 58.86$ meV $= 0.0589$ eV로, 관측 상한($< 0.12$ eV, Planck+BAO)과 양립한다. 질량 제곱차도 직접 도출된다.

- $\Delta m_{21}^2 = 7.33 \times 10^{-5}$ eV²: 관측 $7.53\times 10^{-5}$, **−2.69%**
- $\Delta m_{31}^2 = 2.498 \times 10^{-3}$ eV²: 관측 $2.453\times 10^{-3}$, **+1.85%**

NLO 도입으로 $\Delta m_{31}^2$ 잔차는 +9.95%에서 +1.85%로 줄어든다(5.4× 닫음). 정상 계층(NH)을 자동으로 선호한다. 유도 레벨은 **반정량 닫힘(자유 매개변수 0개, 세 세대 동시 ±1.1% 이내)**이다.
...
#### $|V_{ub}| = \alpha_s^{8/3} \cdot F^{1/3}$: SU($d$) 최대 거리 전이 + 위상공간 보정 (2.7% 일치)
...
$$|V_{ub}|_{\text{tree}}^2 = \alpha_s^{2(d^2-1)/d}$$

$d=3$을 대입하면 지수는 $2(9-1)/3 = 16/3$이다.

$$|V_{ub}|_{\text{tree}}^2 = \alpha_s^{16/3} \quad \Longrightarrow \quad |V_{ub}|_{\text{tree}} = \alpha_s^{8/3} = 0.00334$$
...
**위상공간 보정.** 1세대$\to$3세대 전이는 $d=3$개 공간 방향 각각에서 독립적으로 위상공간 증강을 받는다. 단일 방향 증강 인자는 QCD 진공 form factor $F = 1 + \alpha_s D_{\text{eff}} = 1.375$이고, 전체 진폭은 이 곱의 $d$-제곱근이다 (자세한 도출: `docs/2_경로적분과_응용/12_전이구간.md` §7.3):

$$|V_{ub}| = \alpha_s^{8/3} \cdot F^{1/3} = 0.00334 \times 1.112 = 0.00372$$

관측: $|V_{ub}| = 0.00382 \pm 0.00024$. 차이 **2.7%**, $0.4\sigma$. tree-level의 $-12\%$ 잔차가 $F^{1/3}$ 보정으로 4배 개선된다. 유도 레벨: **가설(기하학적 최대 전이 + 위상공간 보정)**.
...
| $u \to b$ | SU($d$) 최대거리 전이 + $F^{1/3}$ | $|V_{ub}| = \alpha_s^{8/3} F^{1/3}$ | 0.00372 | 0.00382 | 2.7% |
...
**논증.** $J$의 크기가 이미 $4\alpha_s^{11/2}$로 결정되었다면, $\sin\delta_{CP}$는 관측 $J$에서 역산된다.
...
관측상 $\delta_{CP} \approx 1.20 \pm 0.08$ rad(PDG)이고, $\pi/2 = 1.571$ rad이다. $\sin(1.20) = 0.932$이므로 $\sin\delta_{CP} = 1$과는 7% 차이가 난다.

이 잔차는 1-2 세대 질량의 섭동적 보정이나 강한 상호작용의 비섭동적 효과(예: 인스탄톤 가스)가 유효 위상을 이동시키는 현상으로 설명될 수 있다. $J$ 불변량의 크기 정합(1.3%)은 근본 대칭성이 $\pi/2$임을 강하게 지지한다.
...
## 11. 10층 -- 힉스 섹터: 포탈 결합과 자체 결합

### 11.1 SM 힉스 파라미터

SM의 힉스 섹터에는 2개의 자유 파라미터가 있다:

| 파라미터 | 관측값 | SM에서의 역할 |
|---|---|---|
| $v_{\text{EW}}$ | 246.22 GeV | 전자약 대칭 깨짐 스케일 |
| $\lambda_H$ | $\approx 0.13$ | 힉스 자체 결합 ($M_H = \sqrt{2\lambda_H}\,v_{\text{EW}} = 125.1$ GeV) |
...
### 11.4 힉스 질량: $M_H = M_Z \times F$ (0.20% 일치)

#### 관계식의 발견

힉스 질량과 Z 보손 질량의 비율이 CE의 폼팩터 $F$와 일치한다:
...
**논증.** SM에서 힉스 질량은 tree-level 값에 복사 보정(radiative correction)이 더해져 결정된다. 지배적 보정은 top 쿼크 루프이다: $y_t \approx 1$, 색 전하 $N_c = d = 3$.

CE에서 이 보정의 구조를 분석한다. 힉스 유효 포텐셜에 대한 QCD-dressed 보정은:
...
- 분모 $F = 1 + \alpha_s D_{\text{eff}}$: QCD 진공 요동에 의한 증강 (힉스 질량에서도 동일하게 등장)
...
| 19 | $|V_{ub}|$ | $\alpha_s^{8/3} \cdot F^{1/3}$ (SU($d$) 최대거리 전이 + 위상공간 보정) | 0.00372 | 0.00382 | **2.7%** | $\pi, 0$ | 가설 |
...
| **확장: 중성미자 (7)** | | | | | | | |
| 21-23 | $m_{\nu_{1,2,3}}$ | $\dfrac{\delta^4 (1-\alpha_s/\pi)}{(16\pi^2)^2 \cdot 32\pi^3 \cdot (1+R)} m_l^{5/8} m_\tau^{3/8}$ ($d=3$ Casimir 멱 + NLO RG) | $m_2=8.566$, $m_3=49.99$ meV; $\sum=58.9$ meV | $8.66$, $50.5$; $<120$ meV | **−1.09%**, **−1.02%**; 양립 | $\pi, 0$ | 반정량 닫힘 |
...
- **식별된 수학적 간극 4개** (경로적분.md 19.2절 참조):
  ...
  - G3: 전이구간 $\tau_*$의 정밀 시점 결정 (현재 $A_s, \eta, T_{\text{CMB}}$ 모두 보정 후 ~1%로 닫힘. `12_전이구간.md` 참조)
...
우주론 텐션: $H_0$ (**닫힘 가설, 지위 Open/Phenomenology** — "99.28% 해소·자유매개변수 0개" 표현 철회. $\delta\epsilon_0=-\delta/\pi$는 독립 유도 없음(동일 $1+\delta/\pi$ 위상은 CKM 게이트에서 wrong-phase control로 기각; 층이 달라 이전되진 않으나 지지도 없음) → $\Omega_{m,\text{today}}=0.343$, 전이 강성 $\xi_{\text{flow}}=\pi^2/2$ = 반주기 위상면적 측도 $(2\pi)^2/8$(비최소 결합 아님). 모델 계통오차($c_s=c/\sqrt3$ 근사, toy $\theta_*$ 매칭)로 유효 정밀도는 수 % 수준, $\Delta H_0=+5.6\pm0.4$. 검증 코드 실행 시 요구 강성 $\approx5.0$이 $\pi^2/2$와 +0.002%, 독립 horizon-entropy 읽기도 같은 $\pi^2/2$로 $H_0=67.25$(유일 생존자) → 교차 확증. EoS의 $\tilde\xi_w=\alpha_s^{1/3}=0.49$와는 별개 양(기호 충돌 해소), `docs/참조/epsilon_제1원리_유도.md` §16.6), $S_8$ (LCDM 대비 $\Delta\chi^2_{f\sigma_8} = -2.91$, KiDS 잔차 0.34σ 완화).
