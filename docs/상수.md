...
| 이것이 어느 층의 식인가 | `Exact / Selection / Bridge / Phenomenology / Open / Open test`와 주석 | 상수마다 논리 지위와 채점 여부가 다름 |
...
| `Phenomenology` | NLO 보정, 동결 시점, 응용 닫힘이 필요한 식 | $T_{\text{CMB}}$, 일부 우주론/질량 식 |
| `Open` | 채점에서 제외한 재개방 항목. raw 식은 실패했지만 후보 읽기 또는 선택 원리를 검증 중 | 현재 상수 scorecard에는 없음 |
| `Open test` | 아직 관측 비교를 점수화하지 않는 비채점 예측. 다음 실험으로 이론을 압박하는 항목 | $\alpha_{\rm spec}$, $r_{\rm tensor}$ |

이 문서의 목적은 45개 식을 모두 같은 세기로 주장하는 것이 아니다. 어떤 식이 오일러 기반 코어에 가깝고, 어떤 식이 브리지 또는 현상론 단계에 있는지를 함께 집계하는 것이다.

특히 4층 이후의 많은 출력은 코어 정본의 직접 `Exact` 귀결이 아니라, 상위층에서 내려온 중간량을 다른 관측량으로 옮기는 `Bridge` 또는 `Phenomenology` 항목으로 읽는다. `Open`과 `Open test`는 scorecard의 $\chi^2$에 넣지 않는다.
...
| $\Omega_\Lambda$ | 3계층 합 $R_{\text{3-layer}}=0.38063$ (닫힌형 $\alpha_s D_{\text{eff}}(1+\varepsilon^2\delta)=0.37789$은 0.72% 차 근사) | 0.6891 | 0.6847 | 0.64% | `Phenomenology` |
| $\Omega_{DM}$ | 3계층 동상 | 0.2623 | 0.2589 | 1.3% | `Phenomenology` |
| $M_H/M_Z$ | $1+\alpha_s D_{\text{eff}}$ | 1.374 | 1.373 | 0.20% | `Bridge` |
| $\|V_{cb}\|$ | $\alpha_s^{3/2}$ LO, NLO projector $\alpha_s^{3/2}(1+\delta/2\pi)$ | 0.04049 LO, 0.04162 NLO | strict avg 0.04153 | LO 6.58$\sigma$ fail, NLO +0.58$\sigma$ | `Phenomenology` |
...
| $n_s$ | $1 - 2/(dD_{\rm eff}12/2)$ | 0.96503 | $0.965 \pm 0.004$ | 0.0$\sigma$ | `Phenomenology` |
...
| $T_{\text{CMB}}$ | Saha + $X_{\rm dec}=\pi^{-5}$ + 보정 $\eta$ | 2.7295 K | 2.7255 K | 0.15% | `Phenomenology` |
| $A_s$ | 총응답 raw는 기각; projected residual-drive readout | $7.84 \times 10^{-9}$ raw, $2.104\times10^{-9}$ readout | $2.1 \times 10^{-9}$ | raw 실패, readout -0.53$\sigma$ | `Phenomenology` |
| $\alpha_{\rm spec}$ | $-2/N_e^2$ | $-6.11\times10^{-4}$ | CMB running | next test | `Open test` |
| $r_{\rm tensor}$ | $12/N_e^2$ | 0.00367 | CMB B-mode | next test | `Open test` |
...
- `Open`: 현재 scorecard의 $\chi^2$에서 제외한다. `broken`이 아니라 "채택 후보가 아직 닫히지 않은 상태"다.
- `Open test`: 관측 비교를 아직 점수화하지 않는 예측 손잡이다. 다음 실험/데이터로 검증한다.

전이 보정과 보조 비교(NLO 포함): `2_경로적분과_응용/12_전이구간.md`. $\pi^k$ 계수 유도: `2_경로적분과_응용/13_위상공간.md`. 자기재귀성 대칭과 $d=0$ 경계상: `2_경로적분과_응용/14_자기재귀성_대칭.md`. 곱적 분해 정당화: `2_경로적분과_응용/10_공리_정당화.md`. 상세 분류: `경로적분.md` 19절. 이 표는 "오일러 기반 무차원 코어에서 얼마나 떨어져 있는가"를 함께 보여 주는 집계표다.
...
이전 대비 변동:
- $m_p/m_e = 2d\pi^{N_c+N_w}$: $\pi^5 = \pi^3 \cdot \pi^2$ = SU(3)$\times$SU(2) 위상공간 유도 후보. 현재 판정은 `Bridge`.
- $\mu_N = e\hbar/(2m_p)$: $m_p/m_e$ 유도에 종속되는 복원량. 현재 판정은 `Bridge`.
- $m_n - m_p$: $m_d/m_u = \alpha_s^{-1/3}$ + Cottingham EM 보정. 현재 판정은 `Phenomenology`.
...
$d = 0 \to d = 3$ 전이 해석에 의해 $A_s$를 기술하는 현상론적 경로가 제시되어 있었으나, 자기재귀 사상에서 정확한 암시적 미분
$$
\frac{d\varepsilon^2}{dD}=-\frac{\varepsilon^2(1-\varepsilon^2)}{1-D\varepsilon^2}
$$
을 사용하면 고정점 raw 값은 $7.84\times10^{-9}$로 관측보다 크다. 기존의 $\tau_*<1$ 보정은 이 값을 더 키우므로 폐기한다. 새 readout은 유효 기하 정규화 투영 잔차 $(2/\pi)\sigma^{D_{\rm eff}/(D_{\rm eff}+1)}\varepsilon^2(1-\varepsilon^2)$이며 $2.104\times10^{-9}$까지 내려간다. 판정은 `Exact`가 아니라 `Phenomenology`다. 검산: `examples/physics/primordial_spectrum_readout_gate.py`.
...
### 검증 경로 2: 클라루스장 inverse-correlation / 입자언어 bridge 탐색

CE에서 "클라루스 보손"은 독립 입자를 먼저 확정한 것이 아니라
$m_\xi=m_p\delta^2=29.64757$ MeV를 inverse-correlation scale ansatz로
등록하고 국소 입자언어로 시험하는 `Bridge`다. 현재 CE의 실제 connected
2점함수, 고립 pole, residue와 LSZ는 유도되지 않았다.
$\lambda_{\text{HP}}=\delta^2\simeq0.03160$은 별도의 선택적 portal matching
후보이며, 질량창은 $28.388$--$30.908$ MeV다.
...
질량창 안의 pole-compatible 신호는 실험적 particle-language bridge 후보를
지지한다. 그러나 질량과 유의도만으로 CE field identity, pole residue 또는
spectral positivity를 확인하지 않는다. 해당 질량-결합 영역에서 미발견 또는
배제가 나오면 포탈 결합/국소 스칼라 readout의 `Bridge` 재평가가 필요하다.
이는 코어 클라루스장 자체의 즉각 반증이 아니다.

검산과 판정표: `examples/physics/clarus_boson_search_gate.py`.
...
| X17류 17 MeV 신호 | Clarus pole hit 아님 |
...
BMW/DMZ 2026 결과를 반영하면 과거 \(g-2\) 변칙을 겨냥한 보손 근사값은
역사적 비교값으로 재분류한다. Hessian operator
\(\mathcal H_\gamma=\delta^2S[\gamma]\)를 경로적분의 수렴 구조로 읽는
branch에서는 별도 CE 보손 루프를 넣지 않는 null bridge를 택한다. 다만 격자
QCD가 이 추가 CE 구조를 자동 포함한다는 명제는 operator 정의만으로 증명되지
않으므로 독립 확증이 아니라 `Bridge`다.
