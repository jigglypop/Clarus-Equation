...
### 검증 경로 2: 클라루스장 pole / 입자언어 bridge 탐색

CE에서 "클라루스 보손"은 독립 입자를 먼저 가정한 것이 아니라 클라루스장 2점 상관함수의 pole을 입자언어로 읽은 bridge다. 그 pole의 사전등록값은 $m_\phi=m_p\delta^2=29.64757$ MeV, $\lambda_{\text{HP}} = \delta^2 \simeq 0.03160$이며, 질량창은 $28.388$--$30.908$ MeV다.
...
질량창 안의 pole-compatible 신호가 나오면 클라루스장 상관함수 pole bridge가 강하게 지지된다. 해당 질량-결합 영역에서 미발견 또는 배제가 나오면 포탈 결합/국소 스칼라 readout의 `Bridge` 재평가가 필요하다. 이는 코어 클라루스장 자체의 즉각 반증이 아니다.
