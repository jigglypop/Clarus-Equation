...
1m control throat의 이상적 Casimir 요구량을 맞추는 separation은
...
따라서 필요한 특정 파장은 CE light pole의 약 5708번째 harmonic에
해당한다.

## 4. $Q$와 harmonic의 구분

quality factor $Q$는 resonance linewidth, 저장시간과 정상상태 amplitude를
바꾸지만 선형계의 carrier frequency를 5708배 올리지 않는다. 그러므로

```text
29.65 MeV mode + high Q
```

만으로 169GeV 경계 mode를 만들 수 없다. 필요한 것은

```text
고차 비선형 harmonic generation
+ 169 GeV mode의 경계조건
+ 그 상태의 renormalized negative T_mn
```
...
| 문제 | 특정 파장 공명의 역할 |
|---|---|
| 필요한 microscopic length scale | $169$GeV mode로 형식적 matching 가능 |
| CE 29.65MeV pole과의 동일성 | 실패, 약 5708배 차이 |
...
| 요구 Casimir scale의 특정 파장 | $7.33\times10^{-18}$m / $169$GeV |
| CE light pole의 high-$Q$ 공명만으로 요구 mode 생성 | `REFUTED` |
| 약 5708차 harmonic 또는 독립 169GeV mode | `OPEN` |
| multi-mode anisotropic stress transition | `NEW FRONTIER` |

다음 루프는 pressure ratios를 고정하지 않고 throat Casimir tensor에서 외부
vacuum으로 보간하는 $w_r(r),w_t(r)$를 설계하고, 보존식·Einstein 식·안정성
조건을 동시에 검사해야 한다.
