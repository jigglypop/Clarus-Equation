...
coherent D--T scalar charge $A_DA_T=6$을 포함하면 등록 질량 29.64757 MeV에서
1% 열반응률 증가에 필요한 핵자당 직접 결합은 $g_N=0.0174265$다. 이 문서는
...
오차의 3.39배, singlet 오차의 1.18배다. 따라서 관측 정밀도보다 작다고 버릴 수
...
이고 결과는 `-2.07696 keV`다. 이는 deuteron binding의 약
$9.34\times10^{-4}$이다. 고정된 기존 Hamiltonian에서는 0이 아닌 이동이지만,
