...
이 문서는 등호를 defect functional로 인코딩하는 보편성 주장을 자명한 해집합 인코딩과 정칙성이 있는 농축 정리로 분해한다. 모든 등호가 자동으로 계산 가능하거나 CE 물리 모델로 승격된다는 뜻은 아니며, prior·topology·measurability는 추가 입력이다.

독자는 01–02c의 Gibbs 농축과 04장의 kernel 범주를 먼저 읽으면 된다. 인코딩 정리, 적용 사례, defect의 추가 정보, 변분 선택·계산 경계·범주적 위치를 순서대로 읽으며 표의 `[미완성]` 항목은 증명으로 메우지 않는다.
