...
이 문서는 **최소 유도**다. 즉, 정적 배경에서의 유효 중력상수와 지평선 구조를 평균장 수준에서 닫지만,
...
- 질량은 지평선을 유지하는 쪽으로 작용한다.
- 전하는 지평선을 깨는 방향으로 작용하며 벌점은 $F$다.
- 회전은 더 강하게 지평선을 깨는 방향으로 작용하며 벌점은 $F^2$다.
