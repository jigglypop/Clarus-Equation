...
4. AGI 다리 게이트 (`docs/7_AGI/12_Equation.md` 0.0절)

본 evidence 의 등급 체계는 `7_AGI/12_Equation.md` 0.0절의 다리 게이트 `F1~F4`와 1:1로 짝지어 운영한다.

| 다리 게이트 | 본 문서 근거 절 | 강제 사용 규칙 |
|---|---|---|
| `F1` 메커니즘 결손 (`5_Sparsity.md`) | 2.5, 4.4, 7 (`bridge`) | $p^* = (4.87,26.2,68.9)\%$를 신경 활성/구조/배경의 직접 측정값으로 등치 금지 |
| `F2` 비보존 바이패스 (`12_Equation.md` 4.7) | 3.3 (`bridge`) | 무조건 Lyapunov 수렴 표현 금지. 충분조건 + 수면 복원으로만 |
| `F3` 시간/공간 혼동 (`3_Sleep.md` 6.2) | 3.4 (반복 사상은 `bridge`, 매핑 단위 1.6밤/적용) | 시간 비율을 공간 에너지 분배로 등치 금지 |
| `F4` 의식 환원 (`7_Consciousness.md`) | 7 (관련 모든 환원 주장은 `hypothesis`) | "$\delta_\tau$" 같은 메타인지 지표를 의식의 정량 척도로 hard claim 금지 |
