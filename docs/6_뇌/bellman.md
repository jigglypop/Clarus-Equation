...
>
> 관련 문서: 제2장 `lbo.md` (Bellman → HJB → 확산 → 라플라시안/라플라스–벨트라미, 그리고 설계공간 2차 차분 정규화)
>
> 상위 문서: `agi.md` (벨만/마르코프를 경유하지 않고, CE 3x3+1 격자에서 LBO를 직접 도출하는 AGI 아키텍처. 본 문서의 플레이/설계 2층 구조를 AGI라는 제3층으로 확장)
...
따라서 실무적으로는, 설계 레벨을 DP로 재표현하기보다는 평가 프로토콜(`evaluation.md`)을 고정한 뒤 $\hat J(x)$를 값으로만 평가하는 black-box로 두고, ES 같은 방법으로 탐색하는 편이 단순하고(=KISS) 재현 가능하다.
