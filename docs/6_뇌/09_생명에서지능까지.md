...
첫째, C. elegans 수준의 최소 원시 신경계에서는 binary topology가 아니라 weighted chemical connectivity가 감각-domain을 출력-domain으로 보내는 routing operator로 닫힌다. 둘째, Drosophila에서는 memory 하나가 단독으로 추가되는 것이 아니라 cell type, descending/action, mushroom body, central complex 계열이 함께 분화하고, adult FlyWire 규모에서는 이 공동 분화가 permutation gate를 통과한다. 셋째, zebrafish에서는 실제 calcium activity state가 assembly/recurrent 구조와 행동 frame/direction association까지 닫히지만, continuous speed/turn decoding은 공개 partial/Others 자료에 e2-behavior timestamp bridge가 없어서 data-boundary로 남는다. 넷째, mouse IBL/OpenAlyx에서는 첫 strict Neuropixels session과 motor/striatal multi-probe session이 통과했고, 이어 5개 후보 cross-session panel에서 4개 후보가 choice, first-movement speed, wheel action direction을 모두 통과했다.
...
| Mouse Neuropixels/IBL motor-striatal multi-probe gate | single-session passed | NYU-30 choice BA 0.836748, speed BA 0.673460, wheel direction BA 0.851462 with acronym-by-probe model | repeat across sessions and reduce unknown-bin caveat |
| Mouse Neuropixels/IBL cross-session candidate panel | candidate-panel passed | 4/5 candidates pass all three targets; choice 4/5, speed 5/5, wheel 4/5 | larger registered session panel and unknown-bin reduction |
...
| Mouse IBL/OpenAlyx cross-session generalization | `examples/physics/evolution/mouse_ibl_cross_session_region_generalization_gate.py` | `examples/physics/evolution/mouse_ibl_cross_session_region_generalization_report.md` |
