...
| 분야 | 난제 (Problem) | 적용 필드 ($\Phi$) | 유도된 수정항 | 해결 효과 |
| :--- | :--- | :--- | :--- | :--- |
| **유체역학** | 나비에-스톡스 (Blow-up) | 속도장 $\mathbf{u}$ | $-\alpha \nabla (\nabla^2 \mathbf{u})$ | 난류의 무한 에너지를 억제하여 해의 존재성 보장 |
| **양자물리** | 리만 가설 (Zeta Zeros) | 제타 함수 $\zeta(s)$ | $+\lambda |\zeta''(s)|^2$ | 불규칙한 소수 분포를 평탄화하여 영점 정렬 유도 |
| **생물학** | 단백질 접힘 (Levinthal) | 에너지 지형 $E$ | $+\lambda |\nabla^2 E|^2$ | 복잡한 에너지 장벽을 낮춰 최적 구조로 빠르게 유도 |
| **뇌과학** | 학습 및 수면 (Learning) | 시냅스 가중치 $W$ | Offline Smoothing | 낮 동안 쌓인 과적합(Overfitting) 정보를 일반화 |
| **우주론** | 암흑 에너지 (Dark Energy) | 시공간 계량 $g_{\mu\nu}$ | $+\Lambda_{\text{eff}}$ | 우주 전체의 곡률을 펴려는 힘이 가속 팽창으로 관측됨 |
