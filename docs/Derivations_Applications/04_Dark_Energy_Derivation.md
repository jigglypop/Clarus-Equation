...
이 문서는 CE 광명장(Clarus field) 이론을 **우주론(cosmology)**에 적용하여,
...
- **H1**:  
  - 우주 전체의 “연산 복잡도 밀도”를 낮추는 방향으로  
    광명장이 작용하며, 그 결과로 **가속 팽창과 같은 전역 동역학**이 유도될 수 있다는 가정.
...
여기서 $\rho_\text{supp}$는 광명장에 기인하는 평균 에너지 밀도로,
...
### 4.1.1 (정본과의 연결) 스칼라 광명장 모형에서의 조건
...
의 형태로 합쳐진 뒤,  
광명장의 동역학에 의해 **대부분이 곡률 모드로 “재분배”되고**,  
