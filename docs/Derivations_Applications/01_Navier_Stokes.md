...
이 문서는 CE 광명장(Clarus field) 이론을 **유체역학의 Navier–Stokes 방정식**에 적용하여,
...
- **3장**: CE 광명장 근본 공리(A1–A5)와 정의(D1), 가설(H1)  
...
다음의 표준 공리들 위에 “추가적인 광명장 구조”를 얹는 확장으로 설정한다.
...
CE의 광명장 포텐셜 $\Phi_\text{supp}$는 이 엔트로피·정보 개념과 자연스럽게 연결되지만,  
기존 엔트로피 정의를 대체하지 않고 **보조적인 정보 포텐셜로만 사용**한다.

---

## 3. CE 광명장 근본 공리와 정의, 가설

이 절에서는 CE 광명장 이론에서 사용하는 **근본 공리(A1–A5)**,  
...
이 공리는 “비선택 경로 = 광명장 에너지의 원천”이라는 이후 구조의 출발점이 된다.
...
CE 광명장 공리를 Navier–Stokes 해석·수치해석에 어떻게 결합할지의 **최소 구조**를 정한다.
...
- 그 해를 근사하는 수치장 $\tilde{u}(x,t)$에 대해
  - “어떤 모드가 선택 경로에 가깝고,  
    ...
    광명장 포텐셜 $\Phi_\text{supp}$와 곡률 functional로 정의·측정한다.
...
1. 기준 해 $u(x,t)$와 수치 해 $\tilde{u}(x,t)$에 대해,  
   광명장 functional $\mathcal{S}_\text{NS}[\tilde{u}]$를 최소화하는 방향으로  
...
이 장에서는 Navier–Stokes를 통해 CE 광명장 이론이
