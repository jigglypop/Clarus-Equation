...
$$
P(\text{path}) \propto e^{-R(x)}
$$

*   **곡률 장벽**: 문맥상 맞지 않는(비논리적인) 경로는 그 방향의 공간 곡률($R$)이 급격히 높아지도록 학습됩니다.
*   **자연 소멸**: $e^{-R}$ 공식에 의해, 잘못된 경로의 진폭은 자연스럽게 0에 수렴하여 사라집니다. 별도의 `if`문이나 샘플링 트릭 없이도 논리적 일관성이 유지됩니다.
