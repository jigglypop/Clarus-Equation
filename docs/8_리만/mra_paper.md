...
**검증 결과**: 5 Euler 상수의 2-bit reduction 이 정확히 작동. 4 canonical
positional encoding (NoPE, RoPE, ALiBi, xPos) 가 모두 `EulerCEMinimal` 의
단일 head-type spec 으로 정확히 (≤ 0.5 %p) reproduce 된다.

**부수 발견**:

* **`min_mix` (half ALiBi + half xPos) ≈ 단독 best 와 동등** (−5.5 % vs −5.7
  ∼ −5.8 %). 두 tier-1 type 을 섞어도 외삽 우위 없음 → 실용적으로 head-type
  diversity 가 도움 안 됨.
