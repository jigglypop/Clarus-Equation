...
| (pi, e) | 비트 | 헤드 타입 | 문헌 매핑 (정확 등가) |
|---|---|---|---|
| (0, 0) | `00` | identity | NoPE [Kazemnejad 2023] |
| (0, 1) | `01` | additive decay only | ALiBi [Press 2022], `m_h = 1/ξ_h` |
| (1, 0) | `10` | rotation only | RoPE [Su 2021] |
| (1, 1) | `11` | rotation + additive decay | RoPE + ALiBi (≡ `rope_alibi` § 7.7.2) |

> **라벨링 주의**. 코드의 spec 문자열 `head_types="xpos"` 는 (1,1) 버킷의 별칭으로 유지되지만, 정확한 수학적 등가물은 **RoPE + ALiBi 가산** (≡ § 7.7.2 의 `rope_alibi`) 이다. 진짜 xPos (Sun 2023) 는 q/k 에 대한 **곱셈형** magnitude decay (`q_i ← q_i ρ^i`, `k_j ← k_j ρ^{-j}`) 로 본 환원과 다른 family 다. 정성적 카테고리 ("rotation + 거리감쇠") 가 같아 통상 함께 묶이지만, 본 표에서 (1,1) 의 정확 등가는 `rope_alibi` 로 읽는다.
...
#### 검증 — `tests/test_euler_minimal.py` (24 tests)
...
* Autograd: rotation-only 헤드의 `log_xi` grad 는 정확히 0 (decay gate off 시
  grad path 가 끊김) — uniform fast path / mixed bucketed path 양쪽에서 검증.
* uniform SDPA fast path ≡ mixed bucketed path ≡ head-by-head naive ref 모두
  서로 < 1e-4 일치 (float32 SDPA 오차 범위).
* Packed bitmask (`_pi_mask`, `_e_mask`) 가 per-head float buffer 와 비트 단위
  일치, bucket 분할이 헤드 전체를 정확히 1회 cover.

독립 audit (canonical 식 직접 비교) 결과:

| head-type | canonical 식 | max-abs-diff |
|---|---|---|
| (0,0) | NoPE causal SDPA | **1.19e-7** |
| (0,1) | ALiBi (`m_h = 1/ξ_h`) | **9.69e-8** |
| (1,0) | RoPE (rotated Q,K) | **8.94e-8** |
| (1,1) | RoPE + ALiBi 가산 | **1.19e-7** (vs 진짜 xPos: 1.99e-1) |

이로써 EulerCEMinimal 이 본 환원의 정확하고 정밀한 구현임을 확인. 단 (1,1)
버킷의 정확 등가는 진짜 xPos 가 아니라 **RoPE + ALiBi 가산** 이다 (위 표 주의
참조).

#### 실증 — minimal 변종이 canonical baseline 을 정확히 reproduce

`bench_mra_extrap.py --variants minimal` (1500 step × 3 seed × d_model=64) 결과:

| 2-bit variant | head_types | x32 degrad | canonical 매핑 (정확 등가) | x32 (canonical) | 일치 형태 |
|---|---|---|---|---|---|
| `min_nope` | uniform 00 | **+7.0 %** | `nope` (NoPE) | +7.0 % | 동일 모델 (정확 일치) |
| `min_rope` | uniform 10 | **+47.2 %** | `std_rope` (RoPE) | +47.2 % | 동일 모델 (정확 일치) |
| `min_alibi` | uniform 01 | **−5.7 %** | 순수 ALiBi (Press 2022) | (직접 baseline 미측정) | 동일 모델 — `rope_alibi` 와는 회전 유무 차이 |
| `min_xpos` | uniform 11 | **−5.8 %** | `rope_alibi` (RoPE + ALiBi) | −6.2 % | 동일 모델 (정확 일치, 0.5 %p 분산 내) |
| `min_mix` | half 01, half 11 | −5.5 % | (신규) | — | — |
| `min_all` | one of each 00/01/10/11 | −0.6 % | (신규) | — | — |

> **매핑 정정**. 이전 표에서 `min_alibi ↔ rope_alibi`, `min_xpos ↔ euler_ce_k1`
> 로 적었으나, `rope_alibi` 자체가 RoPE + ALiBi 이므로 정확 등가는 **`min_xpos
> ↔ rope_alibi`** 이다. `min_alibi` 의 정확 등가는 **순수 ALiBi (회전 없음)**.
> `min_xpos ↔ euler_ce_k1` 도 구조 (rotation + 가산 거리감쇠) 가 같지만,
> EulerCE 는 `rope_base = π^D_eff·N` (block-aware) 를, EulerCEMinimal 은
> `rope_base = 10000` (RoFormer default) 를 쓰므로 RoPE 주파수만 다르다 — 외삽
> 거동이 사실상 같으나 식은 동일하지 않다.

**검증 결과**: 5 Euler 상수의 2-bit reduction 이 정확히 작동. 3 canonical
positional encoding (NoPE, RoPE, RoPE+ALiBi) 가 `EulerCEMinimal` 의 단일
head-type spec 으로 정확히 (≤ 0.5 %p) reproduce 된다. `min_alibi` (순수
ALiBi) 는 본 ablation set 에 단독 baseline 이 없어 직접 비교 불가하지만,
`rope_alibi` (= `min_xpos`) 와 0.1 %p 차이로 회전 유무가 length-OOD 에 사실상
무관함을 시사한다.

**부수 발견**:

* **`min_mix` (half ALiBi + half RoPE+ALiBi) ≈ 단독 best 와 동등** (−5.5 % vs
  −5.7 ∼ −5.8 %). 두 tier-1 type 을 섞어도 외삽 우위 없음 → 실용적으로
  head-type diversity 가 도움 안 됨.
* **`min_all` (4 type 골고루) 은 best 보다 약간 worse** (−0.6 %). rotation-
  only 와 nope 헤드가 평균 attention capacity 를 깎음. **실용적 권고**: 모든
  헤드를 단일 tier-1 type (`alibi` 또는 `xpos`) 으로 통일하는 것이 최적.

이는 본 작업의 가장 강한 결정적 결론 — **현존하는 모든 main-stream PE 가
사실상 같은 2-bit taxonomy 의 인스턴스이며, 4 가지 중 단 한 가지 (10 = pure
rotation) 만 외삽 catastrophic**.

#### 7.7.4 후속 — boolean lattice 와 simplex 의 관계 (논문 외부)

§ 7.7.3 의 (P, E) 2-bit lattice 가 cell-state algebra 의 *부분 lattice* 임을 정합화한 결과는 `docs/axium.md` § 1.2a.1 (5상수의 통일 boolean axis 분해) 에 정식화되어 있다. 같은 axium § 1.2a.2 의 정리 (boolean lattice $\{0,1\}^3$ 와 simplex $\Delta^2$ 사이 algebra-preserving bijection 부재) 는 본 § 7.7.3 의 환원이 어휘/의미 axis 로 직접 확장되지 않는 *수학적 한계* 를 명시한다 — 단어가 boolean 비트 배열이 아니라 simplex 위의 점 (continuous, mixing 가능) 을 가져야 하는 이유.

부트스트랩 contraction 의 spectral 정합성은 `docs/6_뇌/graph.md` § 10.3 (Bootstrap-Laplacian Spectral Theorem) 에 별도 정리되어 있으며, 이는 본 ablation 의 length-OOD 결과와는 독립이지만, "PE = 거리 axis 한 개의 단순 환원" 과 "뇌의 표현 = simplex × boolean × spectral" 의 차이를 보이는 reference 다.
