...
이 문서는 모델 출력의 오류를 곡률 proxy로 감시하고 개입하는 추론 설계 후보를 정의한다. 독자는 생성 모델의 logit·정규화·평가셋 기본을 아는 독자를 전제로 하며, 출력은 위험 점수와 intervention 규칙이지 임상 환각이나 뇌 병리의 기제를 설명하는 결론은 아니다.

진단·학습 제약·추론 개입·로짓 조정·benchmark 순서로 읽으며, 점수는 무차원 활성 기반 metric이고 시간축은 token 또는 decoding step이다. 성공은 사실성·거부율·유용성의 사전 고정 baseline, OOD, ablation에서 확인되고 악화되면 해당 억제 가설은 기각된다.

> 관련: `6_뇌/07_수면과복구.md` 5.6절(곡률 정규화 추론), `5_유도/05_Neural_RealityStone_Derivation.md`(곡률 functional), `7_AGI/12_Equation.md` 6.4절(유니타리 사영), `examples/ai/sfe_hallucination_suppressor.py`(기존 구현)
>
...
여기서 환각은 사전 고정한 사실성 평가에서 ground-truth provenance를 가진 정답과 불일치한 생성 출력이라는 operational label이다. 곡률은 오류의 원인이 아니라 residual proxy이므로, label provenance·annotator 합의·평가 timebase가 없으면 이 진단은 미완성으로 남는다.

### 1.1 환각 = 곡률 스티킹

여기서 환각은 임상·철학 용어가 아니라, 사전 정의한 사실성 평가에서 오류를 낸 생성 출력이라는 연산적 label이다. 곡률은 그 오류의 원인이 아니라 측정할 후보 proxy이므로 상관·개입·ablation이 모두 실패하면 이 진단은 기각된다.
...
분류는 오류 유형과 개입 비용을 분리하기 위한 평가 지도다. 표의 CE 대응은 진단 가설이며 false positive와 false negative를 동일 benchmark의 ground-truth label로 함께 세지 않으면 반증할 수 없다.
...
학습 제약은 residual proxy를 손실에 넣는 방법이고, 추론 시 개입과는 다른 입력·출력을 갖는다. 이득은 동일 baseline과 loss-term ablation, provenance가 고정된 holdout에서만 판정한다.
...
추론 개입은 decoding step마다 계산한 residual metric을 입력으로 받아 출력을 바꾸는 규칙이다. OOD에서 사실성은 높아져도 정답 차단이 늘면 false positive 실패이며, 개입 누락은 false negative로 기록한다.
...
실시간 점수는 token 또는 decoding step마다 계산한 위험 proxy로 개입 여부를 정한다. 임계값은 calibration split에서 고정해야 하며, 억제로 사실성은 높아져도 유용성·정답률이 떨어지면 실패로 기록한다.
...
로짓 조정은 후보 token의 상대 점수를 위험 proxy만큼 바꾸는 구현 연산이다. 사실성·거부율·유용성의 절충은 score-only와 intervention ablation, 같은 ground-truth provenance의 baseline에서 함께 측정해야 한다.
...
로짓 조정은 후보 token의 점수를 위험 proxy만큼 바꾸는 구현 연산이다. 이는 사실성·거부율·다양성 사이의 절충을 만들 수 있으므로, baseline과 score-only ablation에서 OOD 성능을 함께 측정해야 한다.
...
병리 명칭은 곡률 failure mode를 직관화하는 생물학적 비유다. 임상 진단·인과 기제·시간척도와의 동일성을 뜻하지 않으며, 모델 residual의 성공·실패 판정은 benchmark label에만 근거한다.
...
마취제가 $\rho > 1$인 발산 체계처럼 수축 조건을 파괴하여 의식이 소실되는 것처럼(`07_수면과복구.md` 8.2절), 네트워크의 곡률 정규화를 완전히 제거하면 출력 안정성이 약해질 수 있다.

---

## 6. 곡률 위험 정량화와 벤치마크

위험 metric은 지정한 창에서 residual과 개입을 요약하는 무차원 점수다. 점수 자체가 ground truth가 아니므로 오류 label과의 상관, calibration provenance, false positive·negative, OOD 분할을 분리해 평가한다.

### 6.1 CE 곡률 위험 지표

위험 점수는 지정한 창의 개입 빈도를 세는 무차원 metric이며, 환각의 확률 또는 뇌 상태의 임상 지표가 아니다. 오류 label과의 상관, threshold calibration, 개입 ablation이 없으면 score의 해석은 미완성이다.
...
벤치마크는 사실성·유용성·과도 거부를 분리해 억제의 이득과 비용을 동시에 판정해야 한다. 다음 표는 등록할 측정 계약이며, 단일 in-distribution 점수로 일반화를 주장하지 않는다.
...
gate는 metric 개선이 안전한 억제인지 확인하기 위한 사전 조건이다. 각 조건은 OOD와 component 제거에서 반복되어야 하며 하나라도 깨지면 해당 효능 주장은 보류한다.
...
실패는 proxy·threshold·개입 연산 중 어느 요소가 깨졌는지 분리해 해석한다. residual과 오류가 상관하지 않거나 OOD에서 false positive가 증가하면 해당 억제 가설은 유지하지 않는다.
...
이 절은 입력 토큰에서 residual/gate 출력까지의 producer·consumer와 update 순서를 구현 관점에서 고정한다. 도식은 shape와 데이터 흐름의 contract이며, 환각 억제 효과는 ground-truth fixture·baseline·false positive/negative 검사로 별도 판정한다.
...
운영 원칙은 online risk gating의 입력·출력·감사 기록을 정하는 구현 명세다. 자동 억제가 유익한지는 ground-truth가 지연 도착하는 holdout과 기준선·ablation 비교에서만 판정한다.
