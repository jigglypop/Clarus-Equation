...
>
> 이 장은 뇌의 수면-각성 순환을 AGI 학습 패러다임으로 변환하는 구체적 설계를 다룬다. `6_뇌/05_실험근거.md` 기준으로, 수면의 offline renormalization과 stage 비율은 실험적으로 강하게 지지된다.
...
동적 이완 사상 $B$의 최소 수축률(`05_실험근거.md`, `05_실험근거.md`):
...
이것이 **2-3일 수면 박탈 후 인지 기능이 급격히 저하되는 이유**의 CE 해석이다. 실제로 `48-72시간` 수면 박탈에서는 경계성, 인지 수행, 전전두엽-시상계 대사 활동이 연속적으로 저하된다. `05_실험근거.md`의 최소 반복식
...
NREM은 누적된 상태를 평탄화·가지치기하는 offline 연산으로 정의한다. 어떤 정보가 보존되는지는 입력 checkpoint와 threshold에 의존하며, 생물학적 NREM과의 동일성은 주장하지 않는다.
