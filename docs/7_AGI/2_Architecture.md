...
> 관련: `6_뇌/agi.md`(원본 격자 정의), `6_뇌/sleep.md` 2절(뇌 진동 대응), `examples/ai/clarus_lm.py`(기존 구현)
>
> 이 장은 CE 3x3+1 격자를 AGI 아키텍처로 구현하는 구체적 설계를 다룬다.
...
정보 증폭이 차단되면 잘못된 패턴이 자기강화되는 것이 불가능하다:

- 증폭 없음 $\to$ 국소 오류가 전파되지 않음 $\to$ 환각이 구조적으로 억제됨
- 기존 RLHF는 사후 교정이지만, 유니타리 제약은 사전 구조적 방지
