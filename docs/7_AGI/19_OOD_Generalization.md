...
핵심: **NoPE 가 RoPE 보다 7 배 잘 외삽한다** (+7.0 % vs +47.2 %). 즉 RoPE 의 회전 자체가 OOD 의 직접 원인이며, 이를 보정하는 강한 distance attenuation 이 없으면 회전을 아예 빼는 게 낫다 (Kazemnejad et al. 2023 의 finding 재현).
...
NoPE 가 RoPE 보다 7 배 잘 외삽한다는 사실 (NoPE +7 % vs RoPE +47 %) 은 회전 메커니즘 자체가 OOD 위험 요소임을 보여준다. RoPE 의 phase `θ_k(p) = p · 10000^{−2k/d}` 는 `p` 가 학습 분포 [0, 64] 안에서는 학습된 attention 분포를 만들지만, `p ∈ [64, 2048]` 영역에서는 학습 분포 밖의 phase 를 만든다 — 학습된 attention head 가 본 적 없는 회전 상태에서 평가된다.
