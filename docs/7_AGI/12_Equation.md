...
**[공리: 경험식]** 나머지 성분 $(s,\;b) = (0.2623,\;0.6891)$은 $B_a$의 산출이 아니라 경험식 공리로 도입한다. 물리 정본 `docs/3_상수/9_우주론_수식_의미와_후보.md` §2는 암흑 성분 분할이 별도 stress tensor를 요구한다고 명시하며, `docs/9_등호이전/05o_CE_residual_cosmology_forward_model.md` §1.1은 $\Omega_{\rm DM}/\Omega_\Lambda$ 분할을 경험식으로 등재한다. 사망 조건: $(0.2623,\;0.6891)$을 무입력 전개로 유도하는 정본 문서가 발견되면 이 라벨은 해당 유도로 교체된다(현재 근거: 12-routes B.4의 전 docs grep 음성 — 15.1 표의 "출처 상수 $e,\pi,0$" 표기는 유도 없이 붙은 라벨).
