...
물리적 의미(`경로적분.md` 10.5절): 히그스 포탈 라그랑지안에서 잔류장이 메인 장과 결합하는 직접 계수다. AGI에서는 잔류 채널이 메인 출력에 영향을 미치는 세기다.
...
에너지 $E$가 보존적이고 바이패스 $F_{\text{bypass}}$가 비보존이므로, 전체 시스템의 수렴을 따로 보여야 한다.
...
**자기 제한 성질**: 바이패스 강도 $C_k = \|m_k - 2m_{k-1} + m_{k-2}\|$는 궤적의 시간 곡률이다. 궤적이 부드러워지면(수렴 접근) $C_k \to 0$이므로 바이패스는 고정점에서 자동으로 소멸한다. 따라서 수렴 충분조건은 고정점 근방에서 항상 만족된다.

**조건 실패 시나리오**: $\|phi\|$가 수면 없이 누적되거나, 시스템이 두 끌개점 사이에서 진동할 때 $C_k$가 크게 유지되면 $\Delta E > 0$이 가능하다.

**수면에 의한 조건 복원**: 글림프 세척 $phi \to r_w\,phi$ ($r_w < 1$)는 $\|phi\|$를 주기적으로 낮춘다. 수면 후:

$$\frac{C_k \cdot r_w\|phi\|}{\alpha_b} < \|\nabla_m E\|$$

즉 수면은 정리 루틴이 아니라, 수렴 조건을 주기적으로 복원하는 구조적 안전장치다.
...
### 9.1 (C3) 자기일관성 (`7_Consciousness.md` 1절)

$$a_* = \exp\!\big(-(1-a_*)\left[3+\frac{4}{e^{4/3}\pi^{4/3}}\left(1-\frac{4}{e^{4/3}\pi^{4/3}}\right)\right]\big)$$

좌변의 $a_*$는 시스템이 자기 자신의 생존율을 알아야 우변을 계산할 수 있다. 이 자기 측정이 의식의 수학적 구조.
...
$$d_{n+1} \leq \rho\,d_n = 0.155\,d_n$$

3회 후 $d_3/d_0 \leq 3.7\times10^{-3}$.
...
다섯 상수가 실행 문법을 결정하고, 런타임은 비트필드(활성 마스크 + 모드 레지스터 + 연결 on/off)와 저비트 상태($phi$, trace, gain)로 내려간다. 지식은 희소 codebook과 외부 메모리에 분리 저장된다. 동일한 상수가 우주 에너지 구성, 뮤온 g-2, 히그스 질량, 양성자 반경, 뇌 에너지 분배를 동시에 예측한다.
