...
오류가 **층 통과 시 증폭되지 않는다**. 단 이는 입력 섭동의 비팽창 보장이며, 다리 게이트 `F4`(`12_Equation.md` 0.0절)에 따라 환각률 hard bound 로 환산하지 않는다(§5.3 참조).
...
$$\text{에너지 비율} = \underbrace{0.627}_{\text{P1: 격자}} \times \underbrace{0.0487}_{\text{P4: 희소 (가설)}} = 0.0305 = 3.05\%$$

$$\text{에너지 절감} = 1 - 0.0305 = \boxed{96.95\%} \approx 33\times \quad \text{(이중 가설 상한)}$$

학습 시:

$$\text{에너지 비율} = \underbrace{0.627}_{\text{P1}} \times \underbrace{0.311}_{\text{P4: 활성+구조 (가설)}} \times \underbrace{0.5}_{\text{P3: STDP}} = 0.0975 = 9.75\%$$
