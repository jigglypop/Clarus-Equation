...
$$\frac{dS}{dt} = \begin{cases} \dfrac{1 - S}{\tau_w} & M = \text{WAKE} \quad (\tau_w = 18.2 \text{ h}) \\[8pt] -\dfrac{S}{\tau_s} & M = \text{NREM} \quad (\tau_s = 4.2 \text{ h}) \\[8pt] 0 & M = \text{REM} \end{cases}$$

이산화 ($\Delta t_{\text{mode}} = 1$ min $= 60000$ ms):

$$S^{t+1} = \begin{cases} S^t + \frac{\Delta t_{\text{mode}}}{\tau_w}(1 - S^t) & \text{WAKE} \\ S^t - \frac{\Delta t_{\text{mode}}}{\tau_s} S^t & \text{NREM} \\ S^t & \text{REM} \end{cases}$$
...
$$M_{t+1} = \begin{cases} \text{NREM} & M_t = \text{WAKE} \wedge S^t > C(t) + \theta_S \wedge \|U_t\| < \theta_U \\ \text{REM} & M_t = \text{NREM} \wedge t_{\text{NREM}} > T_{\text{NREM}}(n) \\ \text{NREM} & M_t = \text{REM} \wedge t_{\text{REM}} > T_{\text{REM}}(n) \wedge S^t > S_{\text{cont}} \\ \text{WAKE} & M_t = \text{REM} \wedge t_{\text{REM}} > T_{\text{REM}}(n) \wedge S^t \leq S_{\text{cont}} \\ \text{WAKE} & M_t \neq \text{WAKE} \wedge \|U_t\| > \theta_{\text{alert}} \\ M_t & \text{otherwise} \end{cases}$$
