...
이 문서는 (i) CPMG vs CE 시퀀스 비교 데이터로부터, (ii) 필터 함수 이론을 이용해, (iii) 노이즈 스펙트럼의 유효 구조를 역산/모델 비교하는 절차를 정리한다.
...
- CE는 저주파 모멘트 억제로 **항상** CPMG보다 우수

**실험 관측**:
- $T=10\,\mu\text{s}$: CPMG > CE ($r = 0.54$)
- $T=20\,\mu\text{s}$: CE > CPMG ($r = 1.21$)
...
- **$T \sim 20\,\mu\text{s}$** ($1/T \sim 0.05\ \mu\text{s}^{-1}$):  
  저주파 배경이 지배적 → CE의 저주파 모멘트 억제 효과 → **CE 우세**
...
CPMG 대비 CE의 상대 이득을 주파수 함수로 정의:
$$
G(\omega) \equiv \frac{|Y_{\text{CPMG}}(\omega)|^2}{|Y_{\text{CE}}(\omega)|^2}
$$

- $G(\omega) > 1$: CE가 해당 주파수를 더 잘 억제
...
전체 디코히어런스 비는
$$
r = \frac{\chi_{\text{CPMG}}}{\chi_{\text{CE}}} = \frac{\int d\omega\, S_\Phi(\omega) |Y_{\text{CPMG}}|^2}{\int d\omega\, S_\Phi(\omega) |Y_{\text{CE}}|^2}
= \frac{\int d\omega\, S_\Phi(\omega) G(\omega) |Y_{\text{CE}}|^2}{\int d\omega\, S_\Phi(\omega) |Y_{\text{CE}}|^2}
$$

즉, **$S_\Phi(\omega)$와 $G(\omega)$의 곱의 가중 평균**으로 결정된다.

- $S_\Phi(\omega)$가 저주파 집중 → $G > 1$ 영역 지배 → $r > 1$ (CE 우세)
...
$N$개 duration $T_1, \dots, T_N$에서 측정한 $\chi_i^{\text{obs}}$ (CPMG, CE 각각)에 대해,
...
여기서 가중치는
$$
W_{ik}^{\text{CPMG/CE}} = \int_{\omega_{k-1/2}}^{\omega_{k+1/2}} \frac{d\omega}{2\pi} |Y_{\text{CPMG/CE}}(\omega, T_i)|^2
$$
...
- **CPMG + CE 동시 피팅**: 두 시퀀스의 필터 함수 차이가 $\mathbf{W}$ rank를 높임
...
CE/CPMG 펄스는 $T_{\text{idle}}$ 구간에 적용된다.
...
실험 데이터에서 CE/CPMG 비 $r(T)$를 보면:
- $T=10\,\mu\text{s}$: $r=0.54$ → CE 불리
- $T=20\,\mu\text{s}$: $r=1.21$ → **CE 최적**
- $T=50\,\mu\text{s}$: $r=0.99$ → 차이 없음

따라서 **$T_{\text{idle}} \approx 20\,\mu\text{s}$가 CE 스윗 스팟**이다.

만약 QEC 사이클이 $T_{\text{syn}} + T_{\text{idle}} = 10 + 20 = 30\,\mu\text{s}$라면,  
distance $d=5$에서
$$
\frac{P_L^{\text{CPMG}}}{P_L^{\text{CE}}} \approx 1.21^3 \approx 1.77
$$
즉, **CE가 논리 오류율을 1.77배 감소**시킨다.
...
- **시퀀스**: 각 $N_p$에 대해 CPMG vs CE 최적 시퀀스
- **총 회로 수**: $4 \times 4 \times 2 = 32$ circuits

### 7.2 예상 결과

- **낮은 $N_p$** (4펄스):  
  필터 함수가 저주파 억제 약함 → CE 이점 작음

- **중간 $N_p$** (8–16펄스):  
  CE의 저주파 모멘트 억제 본격 발휘 → $r > 1$ 구간 확대
...
$$
r(\tilde{T}) = \frac{\int d\tilde{\omega}\, \tilde{S}(\tilde{\omega}) |\tilde{Y}_{\text{CPMG}}(\tilde{\omega}, \tilde{T})|^2}{\int d\tilde{\omega}\, \tilde{S}(\tilde{\omega}) |\tilde{Y}_{\text{CE}}(\tilde{\omega}, \tilde{T})|^2}
$$
...
다른 장치에서도 $\tilde{T} \sim 4$ 근처에서 CE 스윗 스팟이 나타날 것으로 예상된다.
...
- **CE 스윗 스팟 자동 탐색 알고리즘**: 런타임 calibration 포함
...
본 절은 CE 이론을 **실험 데이터 기반의 정량적 도구**로 진화시키는 토대를 마련했다.
