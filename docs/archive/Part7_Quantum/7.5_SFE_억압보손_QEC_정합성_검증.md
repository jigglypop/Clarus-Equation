# 7.5 CE 광명보손과 QEC 정합성 검증: 저에너지 허용창 사영

## 1. 개요

Part3에서 뮤온 g-2, 양성자 반경 퍼즐, 우주론 제약을 모두 고려한 결과, CE 광명보손의 고에너지 모드는 $m_\phi \sim 10\text{–}20\ \text{MeV}$, $g_\mu \sim 6\times10^{-4}$ 근처의 좁은 "황금 구역(Sweet Spot)"에서만 살아남았다. 그러나 동일한 광명장이 **저에너지 축에서 만드는 집단 모드(TLS류 저주파 요동)**가 양자 컴퓨팅 노이즈 환경, 특히 QEC와 어떻게 상호작용하는지는 별도의 검증이 필요하다.
...
같은 광명장의 다른 모드 브랜치는 우주론적으로 매우 낮은 에너지를 가질 수 있다 (3.4장 5.3절):
...
```rust
pub const CE_PHASE_SCALE: f64 = 0.01;
...
환경변수 `CE_SUPPRESSON_ENABLE=0`으로 광명보손 모드를 완전히 끈 상태:
...
fn configure_fez_suppresson(noise: f64, steps: usize) {
    ...
    let amp_top = spec.tls_amp * dt_us / (noise * CE_PHASE_SCALE);
...
`CE_SUPPRESSON_ENABLE=2` 모드에서 $(\omega, A)$ 를 수동 지정하여 3×3, 8×2 격자 스캔 실행.
...
### 5.1 광명보손 저에너지 모드의 큐비트 결합

광명보손 저에너지 모드 하나(질량 $m_{\text{low}}$, 에너지 밀도 $\rho_{\text{mode}}$)가 큐비트에 유도하는 위상 노이즈 진폭은
...
- $\rho_{\text{mode}}$ : 저에너지 모드 에너지 밀도
  - 전체 광명장 밀도 $\rho_\Phi \sim \rho_\Lambda \sim (2.3\ \text{meV})^4 \approx 3\times10^{-11}\ \text{eV}^4$
...
즉, **전체 광명장 에너지의 10억분의 1조분의 1 정도만 저주파 TLS 모드에 배분되면** 충분하다.
...
- **고에너지 축** (Part3):
  - 뮤온 g-2: $\Delta a_\mu^{\text{CE}} \approx 2.51\times10^{-9}$
...
CE 광명장의 다중 모드 구조(3.4장 6.4절, 10.2절)에서:
...
세 축을 **단일 광명장 프레임워크 안에서 동시에 설명**할 수 있는 정합적 구조임을 시사한다.
...
```bash
cd sfe_core
CE_SUPPRESSON_ENABLE=0 cargo run --release -- qec -d 5 -n 0.10
```
...
```bash
cd sfe_core
CE_SUPPRESSON_ENABLE=2 \
CE_SUPPRESSON_OMEGA=0.008 \
CE_SUPPRESSON_OMEGA2=0.004 \
CE_SUPPRESSON_AMP=8.0 \
CE_SUPPRESSON_AMP2=8.0 \
cargo run --release -- qec -d 5 -n 0.10
```
