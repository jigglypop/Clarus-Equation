...
- 결맞음 시간: $\displaystyle \frac{\tau_D^{\text{CE}}}{\tau_D^{\text{STD}}}=\frac{1}{1-\epsilon_{\text{mass}}}$
- 비대각 감쇠율: $\gamma_{\rm eff}=\gamma_{\rm std}\,(1-\epsilon_{\text{mass}})$
- 상대론적 관측 수명: $\displaystyle \frac{\tau_{\text{obs}}^{\text{CE}}}{\tau_{\text{obs}}^{\text{STD}}}=\frac{1}{\sqrt{1-\epsilon_{\text{mass}}}}$
- 분광/반동/고유량: 불변
...
- CSL 구분: 질량 스케일링 곡선이 CE의 고정 배율과 다르면 CSL류(질량 의존 붕괴율) 신호.
...
- 모델: $T_2\propto 1/Y$로 가정할 때, CE 유래 항은 $\propto \epsilon_{\text{mass}}^{\,2}$.
