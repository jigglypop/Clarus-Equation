# 18장: 광명장의 상호작용 해석과 암흑에너지 대체

## 개요

본 장에서는 광명장(Clarus field)의 물리적 본질을 재해석한다. 광명장은 독립적인 스칼라장이 아니라, **우주 전체에 분포한 모든 양자 간 상호작용의 집합적 효과**임을 보이고, 이를 통해 암흑에너지를 필요로 하지 않는 우주론을 구성한다.

### 핵심 주장

1. 광명장 $\Phi(\mathbf{x}, t) = \int d^3x' \, \rho(\mathbf{x}', t) \, G(\mathbf{x} - \mathbf{x}')$
   - $\rho$: 우주 전체 물질/에너지 밀도
   - $G$: 상호작용 커널 (비국소적)
...
- $G(\mathbf{x} - \mathbf{x}', t)$: 비국소 상호작용 커널
  ...
  18A장에서는 이 $\alpha_{\text{SI}}$가 **순수 자연상수** $G_N, c, \hbar, m_p, m_e, \alpha_{\text{EM}}$로부터 어떻게 유도되는지를 자세히 보였다. 특히 1.2장에서 정리한 것처럼, CE는
  $$
  M_{\text{Pl}}^2 \equiv \frac{1}{8\pi G_N} \propto \Phi_0^2
  \qquad\Longleftrightarrow\qquad
  G_N \propto \frac{1}{\Phi_0^2}
  $$
  라는 중력–광명장 역제곱근 비례관계를 가정한다. 18A장의 결과
  $$
  ...
  $$
  를 이 관점에서 보면 $\alpha_{\text{dimless}}\propto 1/\sqrt{G_N}\propto \Phi_0$ 이므로, 커널의 결합 상수 $\alpha_{\text{SI}}$는 **광명장 진공값(=중력 스케일)을 전자기·양자 스케일과 결합시킨 양**으로 해석된다. 즉 위 커널 $G(\mathbf{r},t)$는 "단순한 Yukawa 꼴"이 아니라, **중력–광명장–QED가 결합된 제1원리 상수**를 계수로 갖는다는 점이 이론의 핵심이다.
...
### 2.2 광명장 해석: 배경과 들뜸의 이중 역할

**핵심 가설**: 암흑에너지와 암흑물질은 실재하는 별개의 실체가 아니라, 광명장 $\Phi$의 **두 가지 다른 상태**이다.

**광명장의 이중 구조**:

CE 이론에서 광명장 $\Phi$는 두 가지 형태로 존재한다:
...
- CE 이론: 둘 다 광명장 $\Phi$의 다른 상태, **단일 물리적 기원**
...
1. **가정**: 우주는 물질($\rho_m$) + 복사($\rho_r$) + 광명장($\Phi$)만 존재
   - 암흑에너지 $\rho_\Lambda = 0$
...
광명장 해석이 $\Lambda$CDM과 다른 예측:

1. **적색편이 의존 $w(z)$**:
   - $\Lambda$CDM: $w = -1$ (정확히)
   - CE: $w(z) = -1 + \delta w(z)$, $|\delta w| \lesssim 0.05$

2. **초기 우주 섭동**:
   - 광명장은 $z \gg 1$에서 무시 가능
   - CMB: 차이 없음 
...
광명장이 중력과 양자역학을 연결한다면, 자연스러운 스케일:
$$
...
print(f"  광명장 예측:  {rho_Phi_calc:.2e}")
...
plt.subplot(2, 2, 2)
plt.plot(z_array, w_array, 'g-', label='CE 예측')
