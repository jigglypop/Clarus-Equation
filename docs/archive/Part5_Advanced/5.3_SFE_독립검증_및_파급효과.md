...
$$
\frac{\Delta \alpha_{\text{EM}}}{\alpha_{\text{EM}}} \sim \frac{\Delta \epsilon}{\epsilon} \sim \frac{\Delta \alpha_{\text{CE}}}{\alpha_{\text{CE}}}
$$
...
$$
e^2 \to e^2(1 - \delta), \quad \delta \sim \epsilon \sim \alpha_{\text{CE}}
$$
...
광명장 일정 ($\Phi_{\text{local}} = \Phi_{\text{global}}$):
...
광명장 시간 변화 → ISW (Integrated Sachs-Wolfe):
...
$$
\Phi_{\text{CE}} \propto \alpha(z) H(z)^{-2}
$$

시간 미분:

$$
\frac{d\Phi_{\text{CE}}}{dt} \propto \frac{d\alpha}{dt} H^{-2} + \alpha \frac{d(H^{-2})}{dt}
$$
