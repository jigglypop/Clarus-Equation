...
리만 기하학 기반 표현 공간에서 CE 광명장을 포함한 인공지능 구조를 정의한다. 이 구조는 기존 선형층 기반 신경망과 달리, 곡률에 따른 자동 안정화와 경로 억압을 내장한다.
