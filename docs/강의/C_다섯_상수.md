...
이 문서에서는 유도 강도를 아래처럼 구분한다.

- **정리**: 수학적 전개만으로 닫히는 단계
- **규약**: 단위/정규화 선택 단계
- **가정**: 물리적 식별(관측량 대응) 단계

같은 식이라도 어느 레벨에서 성립하는지 분리해서 읽어야 "완전 증명"과 "모형 선택"을 혼동하지 않는다.
...
$\lambda$는 "자유도 단위"를 정하는 계수이므로, SFE는 $D$의 단위를 1 e-fold 기준으로 정규화하여 $\lambda=1$을 택한다. 이 단계는 **규약**이다.
...
유도 레벨: (i)--(iv) **정리**, (v) **정리 7.2 + 보조정리 7.4 + 부트스트랩 등분배 정리** (경로적분.md 3.2.1-보강2). A3는 가정이 아니라, 유클리드 등분배 + 부트스트랩 자기일관성의 구조적 귀결로 유도된다.
...
로 수렴한다. 즉, 본 문서의 "외부 입력 0개"는 $d=3$, $\alpha_{\text{total}}=1/(2\pi)$ 이후 추가 측정 입력이 없다는 뜻으로 사용한다.
...
SFE는 강한 CP 문제를 세 가지 독립 논증으로 해결한다:

1. **실수 스칼라 보호:** 억압장 $\Phi$는 실수 게이지 단일항이므로 CP 위반 위상을 도입하지 않는다.
...
**유도.** CKM 원소 $V_{cb}$는 charm(2세대)과 bottom(3세대) 약한 고유 상태 사이의 전이 진폭이다. SFE에서 세대는 접힘 헤시안의 고유 모드이다.
...
렙톤 혼합은 CKM과 달리 큰 혼합각을 보인다. SFE에서 이것은 중성미자 질량이 복사적으로 생성되어(9.6절) 질량 행렬의 구조가 "민주적"이기 때문이다.
...
여기서 $1/d$는 3세대 민주적 혼합, $1/N_w$는 2중항 구조의 최대 혼합이다. 이것은 Harrison-Perkins-Scott(2002)의 TBM 패턴과 동일하며, SFE에서 $d = 3$과 $N_w = 2$로부터 결정된다.
...
핵심 가정: EWSB 섭동이 SU($d$)의 모든 생성원에 민주적으로 분배된다. 이것은 SFE에서 억압장 $\Phi$가 플레이버 단일항(flavor singlet)이라는 조건의 직접적 결과이다. 유도 레벨: **가설**.
...
$v_{\text{EW}}$는 SFE의 유일한 **차원 입력**이다. $v_{\text{EW}}$는 페르미 상수로 결정된다:

$$v_{\text{EW}} = (\sqrt{2}\,G_F)^{-1/2}$$

SFE의 모든 무차원 예측은 $\{e, \pi, i, 1, 0\}$만 필요하다. $v_{\text{EW}}$는 $G$, $\hbar$, $c$와 함께 "단위를 물리 세계에 부착하는" 차원 규약이다. SFE의 철학에서 이것은 미터를 피트로 바꾸는 것과 같다 -- 구조를 결정하지 않는다.
...
히그스 질량과 Z 보손 질량의 비율이 SFE의 폼팩터 $F$와 일치한다:
...
SFE에서 이 보정의 구조를 분석한다. 히그스 유효 포텐셜에 대한 QCD-dressed 보정은:
...
이 기하급수 합산은 SM의 leading-log 재합산(resummation)과 동일한 구조이며, SFE에서 $F = 1 + \alpha_s D_{\text{eff}}$가 양성자 반경에서도 동일하게 등장하는 것(경로적분.md 15.10절)은 독립적 교차 검증이다.
...
유도 레벨: **가설** (1.1% 수치 정합, 0개 자유 파라미터, $N_{\text{gauge}} = d(d+1)$의 구조적 의미 존재).
...
| # | SM 파라미터 | SFE 유도 공식 | 예측값 | 관측값 | 차이 | 상수 | 레벨 |
|---|---|---|---|---|---|---|---|
| **게이지 결합 (3)** | | | | | | | |
...
아래 체크리스트는 "어떤 외부 데이터/계산이 어떤 입력 오차를 줄이고, 그 결과 어떤 SFE 출력 오차를 개선하는가"를 일대일로 매핑한다.
...
- **식별된 수학적 간극 4개** (경로적분.md 19.2절 참조):
  ...
  - G2: $P_{\text{survive}} = \Omega_b$ 식별 (유일성 정리로 구속, 반증 가능)
...
우주는 이 한 줄의 전개이다. 간극은 이론의 결함이 아니라 다음 단계의 과제이다.
