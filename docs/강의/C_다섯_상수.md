...
**(i)** 경로적분에서 광명장 $\Phi$에 의한 생존 확률을 정의한다: $P_{\text{survive}} = \langle e^{-\Phi} \rangle$.
...
- **진공 에너지** (0차): $\Omega_\Lambda$ -- 광명장 기저 상태의 에너지 밀도
- **양자 요동 응축** (1-루프): $\Omega_{DM}$ -- 광명장 요동이 형성하는 비상대론적 응축체
...
CE가 그 이유를 제공한다. 이 공식이 아름다운 이유는 **우주의 완전한 기술이기 때문이다**. 다섯 상수의 관계 하나에 우주 전체의 무차원 구조가 들어있다. 인간의 미적 감각이 이것을 감지한 것이다.
...
CE 경로적분은 $d$차원에서 곱적으로 분해된다(P1). 차원당 결합 = $\alpha_s^{1/d}$. 전자약 혼합이 이중선형(2-vertex) 게이지 과정이고(P2), SU(2) 기본 표현 차원이 $N_w = 2$이고(P3), 색 전하 수 = 공간 차원 $N_c = d = 3$이면(P4):
...
1. **실수 스칼라 보호:** 광명장 $\Phi$는 실수 게이지 단일항이므로 CP 위반 위상을 도입하지 않는다.
...
핵심 가정: EWSB 섭동이 SU($d$)의 모든 생성원에 민주적으로 분배된다. 이것은 CE에서 광명장 $\Phi$가 플레이버 단일항(flavor singlet)이라는 조건의 직접적 결과이다. 유도 레벨: **가설**.
...
1. **단일 위상 원천.** 10.3절에서 광명장 $\Phi$는 실수 스칼라이므로 새로운 CP 위상을 도입하지 않는다. 따라서 물리적 CP 위상은 페르미온 Yukawa 블록(쿼크/렙톤) 재위상 불변량에만 남는다.
2. **전블록 실수성 조건.** 실수 광명장 배경에서 전체 유효작용의 복소 위상은 쿼크 블록과 렙톤 블록이 상쇄되어야 한다:
