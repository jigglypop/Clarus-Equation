# Length-OOD reproduction — historical results

> **보존 상태:** 이 기록이 가리키던 `paper/7_AGI/19_OOD_Generalization.md`와
> `experiments/ood_length_repro.py`는 현재 체크아웃에 없다. 아래 수치와 명령은 과거
> 실행 영수증이며, 실행 모듈과 입력을 복구하기 전에는 현행 재현 근거로 사용하지 않는다.
...
과거 실행 명령(현재 실행 불가):
`SEEDS=0,1,2 STEPS=500 EVAL_LENS=64,256,1024,2048 python -m experiments.ood_length_repro`
