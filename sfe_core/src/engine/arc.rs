...
impl SfeArcController {
    ...
    pub fn step(&mut self, measured_r: f64, dt: f64) -> f64 {
        ...
        -(self.alpha * sqrt_r + self.beta * pred_v)
    }
}
...
impl CurvaturePredictor {
    ...
    fn update(&mut self, measured_r: f64, dt: f64) {
        ...
        let pred_v =
            self.est_v + (-self.gamma * self.est_v - self.omega_0.powi(2) * self.est_r) * dt;
...
impl ArcSimulationEnv {
    ...
    pub fn step(&mut self, dt: f64) -> (f64, f64) {
        ...
        let actual_noise_influence =
            self.controller.alpha * r_clamped.sqrt() + self.controller.beta * self.true_v;
