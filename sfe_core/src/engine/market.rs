...
impl CeTradingBot {
    ...
    pub fn process_tick(&mut self, price: f64, volume: f64, high: f64, low: f64) -> f64 {
        ...
        if self.position == 0.0 {
            ...
            } else if price < self.entry_price * 0.90 {
                // 손절 폭 10%로 확대
    ...
    fn sell(&mut self, price: f64, reason: &str) {
        ...
        self.holdings = 0.0;
        self.position = 0.0;
        println!("  [SELL] Price: {price:.2} ({reason})");
    }
...
impl MacroEconomy {
    ...
    pub fn update_and_predict(
        ...
        let growth_power = (self.gdp_growth
            + self.m2_growth * 0.5 * self.beta_liquidity * liquidity_efficiency)
            .max(0.0001);
