...
def linearize_with_circuit_response(
    ...
    for tick in range(fixture.horizon):
        ...
        direct_transition_dot = np.zeros_like(transition)
        for receiver in range(q):
            ...
            for sender in range(q):
                ...
                direct_transition_dot[receiver, column] += (
                    fixture.update_fraction[receiver]
                    * gain[receiver]
                    * fixture.circuit[receiver, sender]
                    * efficacy
        ...
        direct_transition_dots.append(direct_transition_dot)
    ...
    return {
        ...
        "direct_transition_dots": direct_transition_dots,
...
def run_seed(fixture: Fixture) -> dict[str, Any]:
    ...
    # The direct-edge-only partial derivative deliberately omits the
    # trajectory-mediated phi''(h) dot(h) term.  It is diagnostic only.
    _, direct_partial_product_dot = transition_and_response_from(
        baseline["transitions"], baseline["direct_transition_dots"], 0
    )
    projection, injection = selectors(fixture)
    direct_partial_jdot = projection @ direct_partial_product_dot @ injection
    direct_partial_error = normalized_error(direct_partial_jdot, fd_jacobian_dot)
    ...
    passive_status = (
        "PASSIVE_FULL_RANK"
        if jacobian_rank == fixture.q
        else "PASSIVE_RANK_UNCERTIFIED"
    )

    seed_pass = all(
        (
            tangent_error <= TANGENT_TOL,
            jacobian_rank == fixture.q,
            circuit_infinity_norm <= 0.48,
            max_abs_weight_path <= 0.47,
    ...
    return {
        ...
        "weight_domain": {
            ...
            "circuit_infinity_norm": circuit_infinity_norm,
            "domain_gate_pass": bool(
                circuit_infinity_norm <= 0.48 and max_abs_weight_path <= 0.47
            ),
        },
        "passive": {
        ...
        "circuit_response": {
            ...
            "direct_edge_only_partial_error_diagnostic": direct_partial_error,
        },
...
def main() -> int:
    ...
    contract_path = source_path.parent.parent / "00-contract.md"
    result = {
