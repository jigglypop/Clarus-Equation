'''Reproduce the E68 scalar cubic interaction-Hamiltonian Legendre gate.'''
...
from examples.physics.qft_reference_flrw_cubic_dynamics import (
    evaluate_scalar_interaction_hamiltonian,
)
...
def main() -> None:
    ...
    for wavenumber in args.wavenumbers:
        receipt = evaluate_scalar_interaction_hamiltonian(
            state,
            parameters,
    ...
    print(
        json.dumps(
            {
                'all_declared_legendre_gates_passed': all(
                    item['declared_legendre_gate_passed'] for item in receipts
                ),
