"""닫힌 라벨 경첩(hinge) 위의 정확한 유한 비평탄 플레바인스키(Plebanski) 증인.

앞선 두 단체 CE 증인은 모든 공유 삼각형 둘레의 링크(link)가 열려 있어,
그 자체로는 닫힌 레게(Regge) 쌍대 면을 지탱할 수 없다. 이 모듈은 원래 4-단체
둘을 보존하고, 고정된 여섯 꼭짓점 집합 위에 빠진 최소 세포 하나

    (0, 1, 2, 3, 5)

를 추가하여 ``f=(1,2,3)`` 의 링크를 3-순환 ``0--4--5--0`` 으로 닫는다. 같은
라벨 꼭짓점 위에 무차원 곡률 ``kappa = K L_ref^2`` 를 가진 입체사영
(stereographic) 드지터(de Sitter) 사면체틀(tetrad)을 설치하고
...
닫힌 원(primal) 삼각형 홀로노미(holonomy)와 추상 쌍대 고리의 여섯 구간 사영
무게중심 실현 둘 다 같은 상수 곡률 접속으로 계산한다. 후자는 레게 결손각과
동일시하지 않는다. 이는 유한 비평탄 조건부 존재 증명서이다. 모든 점에서의
장 항등식은 해석적이고, 런타임 점 평가는 수치 회귀일 뿐이다. 이 증명서는
``kappa`` 를 맨 0D 자료에서 유도하지 않으며, 고유 스핀폼(spin-foam) 진폭이나
측도를 고르지 않고, 세분 수렴을 증명하지 않으며, 중력자 두 자유도를 유도하지
않는다.
"""
...
def _permutation_parity(indices: tuple[int, ...]) -> int:
    """첨자열의 순열 홀짝 부호를 반환한다. 중복이 있으면 0이다."""
...
def _simplex_oriented_coordinate_volume(
    ...
    """4-단체의 좌표 모서리 행렬식으로 방향 있는 부피를 반환한다."""
...
def _hinge_link_edges(
    ...
    """경첩을 포함하는 각 4-단체가 링크에 더하는 모서리 집합을 반환한다."""
...
def _connected_graph(
    ...
    """모서리 집합이 만드는 그래프가 연결되어 있는지 반환한다."""
...
def curved_hinge_trace_audit(
    *,
    ...
    """``f=(1,2,3)`` 둘레의 원래 두 세포 링크를 최소로 닫는다."""
...
def _hodge_two_form_conformal_lorentz(two_form: np.ndarray) -> np.ndarray:
    """2-형식의 로렌츠 호지(Hodge) 쌍대를 반환한다."""

    # 4차원에서 2-형식 위의 호지 별 연산은 등각 불변이다.
...
class DeSitterPlebanskiPointAudit:
    """한 점에서 평가한 입체사영 드지터 항등식의 잔차 묶음이다."""
...
class ExactPrimalHolonomyAudit:
    """라벨된 원 삼각형의 정확한 드지터 홀로노미 감사 결과이다."""
...
def exact_primal_triangle_holonomy(
    *,
    ...
    """라벨된 면의 정확한 입체사영 드지터 홀로노미를 반환한다."""
...
def _de_sitter_embedding(
    ...
    """입체사영 좌표를 5차원 민코프스키 공간의 드지터 쌍곡면 위 점으로 매장한다."""
...
def _de_sitter_centre(points: Sequence[np.ndarray], kappa: float) -> np.ndarray:
    """매장된 꼭짓점들의 양의 합을 쌍곡면 위로 정규화한 세포 중심을 반환한다."""
...
def _parallel_transport_matrix(
    ...
    """두 쌍곡면 점을 잇는 측지선을 따른 평행 수송 행렬과 분모를 반환한다."""
...
def _orthonormal_tangent_frame(point: np.ndarray, kappa: float) -> np.ndarray:
    """쌍곡면 위 한 점의 접공간에 대한 (-,+,+,+) 정규직교틀을 반환한다."""
...
class ExactDualHolonomyAudit:
    """추상 쌍대 고리의 사영 무게중심 실현을 따른 홀로노미 감사 결과이다."""
...
def exact_dual_hinge_holonomy(
    ...
    *,
    ...
    """추상 쌍대 고리의 사영 실현을 따라 수송한다.

    세포 ``S`` 에 대해 택한 중심은 매장된 꼭짓점들의 정규화된 양의 주변 합이다.
    모든 기(flag) ``T subset S`` 에 대해 연결 측지선은 ``S`` 의 정규화된 양의
    원뿔 안에 놓인다. 두 중심의 양의 선형 결합은 양의 꼭짓점 가중치로
    전개되기 때문이다. 따라서 아래 여섯 기는 닫힌 링크의 무게중심 쌍대 순환을
    실현한다. 이 정의는 외심 레게 쌍대가 아니며, 그 홀로노미가 레게 결손각과
    같다고 주장하지 않는다.
    """
...
class CurvedPlebanskiHingeCertificate:
    """같은 역사 위의 최소 비평탄 플레바인스키/아인슈타인 증인 증명서이다."""
...
def constructive_curved_plebanski_hinge_witness(
    *,
    ...
    """같은 역사 위의 최소 비평탄 플레바인스키/아인슈타인 증인을 만든다."""
