"""CE 잔차 우주론의 최소 전방 모델(forward model)이다.

``cosmology`` 의 밀도비 감사 다음 단계다. CE 밀도비를 현재 경계 자료로 쓰고, 보수적인
w0-wa/GR 극한에서 배경 팽창, 거리, 선형 성장을 계산한다.

입자 암흑물질 모형이나 검출기 모형은 의도적으로 아니다.
"""
...
from examples.physics.darksector.cosmology import CE_RATIOS, interp_linear, linspace, logspace, simpson  # noqa: E402
...
def sha256_hexdigest(payload: bytes) -> str:
    """풀이기 내보내기가 준 바이트 그대로의 SHA-256 요약을 돌려준다."""
...
def parse_recombination_history_bytes(
    ...
    *,
    ...
    """해시된 CLASS/CAMB/HyRec z,x_e 표를 파싱한다.

    ``#`` 으로 시작하는 주석과 빈 줄은 무시한다. 열 번호는 0부터 센다. ``delimiter`` 가
    ``None`` 이면 공백으로 나눈다. 원시 바이트 SHA-256 은 따로 주어져야 하고, 숫자 행을
    받기 전에 일치해야 한다.
    """
...
def load_recombination_history_table(
    ...
    *,
    ...
    """해시로 불변성을 보장한 외부 재결합 표를 디스크에서 읽는다."""
...
def photon_density_h2(tcmb_k: float) -> float:
    """흑체 온도에서 omega_gamma = Omega_gamma h^2 를 돌려준다."""
    if tcmb_k <= 0.0:
        raise ValueError("tcmb_k must be positive")
...
def radiation_density_h2(tcmb_k: float, n_eff: float) -> float:
    """표준 자유 흐름 상대론적 중성미자를 포함한 omega_r 을 돌려준다."""
    if n_eff < 0.0:
        raise ValueError("n_eff must be non-negative")
...
def early_hubble_rate_s_inverse(z: float, params: CEForwardParams) -> float:
    """기준 복사+물질+Lambda 배경의 H(z) 를 s^-1 단위로 돌려준다."""
...
def hydrogen_nuclei_number_density_today_m3(
    ...
    """선언된 원시 헬륨 질량 분율로 n_H,0 를 m^-3 단위로 돌려준다."""
...
def baryon_drag_inertia_ratio(z: float, params: CEForwardParams) -> float:
    """선언된 CE/온도 입력에 대한 R=3 rho_b/(4 rho_gamma) 를 돌려준다."""
    if not math.isfinite(z) or z < 0.0:
        raise ValueError("redshift must be finite and non-negative")
...
def drag_optical_depth_rate_per_redshift(
    ...
    """x_e=n_e/n_H 에 대한 d tau_drag/dz 를 돌려준다.

    관례는 표준 바리온 오일러 끌림률과 같다.

        tau_drag(z) = integral_0^z [c sigma_T n_e(z')]
                      / [H(z') (1+z') R(z')] dz',
        R = 3 rho_b / (4 rho_gamma),  n_e=x_e n_H,0 (1+z)^3.

    돌려주는 비율은 단위 적색이동당 무차원이다.
    """
...
def eisenstein_hu_drag_redshift(omega_m_h2: float, omega_b_h2: float) -> float:
    """아이젠슈타인-후(Eisenstein-Hu) 바리온 끌림 적색이동 적합식을 돌려준다.

    CE 경계 모형에 들여온 표준 초기 물리의 선택/가정이다. CE 재결합이나 핵물리에서
    내부적으로 유도한 것이 아니며 DESI 벡터에 맞춰 조정하면 안 된다.
    """
...
def baryon_photon_sound_speed_km_s(
    ...
    """R=3*rho_b/(4*rho_gamma) 로 c/sqrt(3(1+R)) 를 돌려준다."""
    if a < 0.0:
...
def sound_horizon_at_redshift_mpc(
    ...
    """선언된 z_drag 까지 공변 광자-바리온 음향 지평선을 적분한다."""
...
def early_universe_sound_horizon(
    ...
    """표준 초기 우주 물리와 심프슨 적분으로 r_d 를 유도한다.

    계산은 CE 밀도 경계값, 외부 H0 와 ``T_CMB``, 표준 물리의 ``N_eff`` 가정을 쓴다.
    복사는 표준 흑체와 자유 흐름 중성미자 관계를 따르고, 끌림 시기는 아이젠슈타인-후
    적합을 쓰며,

        r_d = integral_0^a_d c_s(a) / [a^2 H(a)] da.

    아이젠슈타인-후/재결합 물리는 선택/가정이지 CE 내부 재결합·핵물리 유도가 아니고
    정밀 재결합 계산도 아니다. 적합된 끌림 적색이동은 정밀도 수준에서 r_d 를 치우치게
    할 수 있다.
    DESI 자료는 이 함수의 런타임 입력이 아니다. 이 모드는 DR2 잔차를 본 뒤 추가됐으므로
    DR2 는 손대지 않은 홀드아웃이 아니다.
    합성 심프슨을 위해 ``integration_points`` 는 홀수여야 한다.
    """
    if integration_points < 3 or integration_points % 2 == 0:
        raise ValueError("integration_points must be an odd integer >= 3")
...
def drag_optical_depth_benchmark(
    ...
    *,
    ...
    """풀이기가 내보낸 x_e(z) 역사에서 tau_drag=1, z_d, r_d 를 계산한다.

    외부 표는 재결합만 준다. H(z), R(z), 음향 지평선은 선언된 CE 경계와 표준 복사 가정을
    쓴다. 역사 메타데이터는 그 입력과 일치해야 하고, 원시 표 해시는
    :func:`parse_recombination_history_bytes` 에서 이미 검증됐어야 한다.

    불투명도 비율은 풀이기 격자점마다 계산하고, 적색이동에 대해 조각별 선형으로 보고,
    그 보간 위에서 정확히 적분한다. 재현 가능한 어댑터 기준이지 CLASS/CAMB/HyRec 실행
    자체나 독립적인 정밀 재결합 계산이 아니다.
    """
    if (
...
def sound_horizon_selection(params: CEForwardParams) -> SoundHorizonSelection:
    """이전 호환 외부 r_d 또는 유도된 초기 우주 r_d 를 고른다."""
...
class ParameterProvenance:
    ...
    def is_ce_prediction(self) -> bool:
        ...
        새 닫힘 코드는 :attr:`closure_role` 을 써야 한다. 원래 역할은 옛 점수표와 재현
        테스트가 직렬화했기 때문에 유지한다. 특히 ``ce_prediction`` 은 눈가림 예측이나
        물리적으로 닫힌 예측을 뜻하지 않는다.
        """
        return self.role == "ce_prediction"

    @property
    def closure_role(self) -> str:
        """통합 우주론 게이트가 쓰는 닫힌 실패 과학적 역할이다."""
    ...
    def qualifies_as_physical_prediction(self) -> bool:
        """이 항목이 독립적으로 닫힌 CE 예측인지 여부다."""
        return False


def parameter_provenance(params: CEForwardParams) -> tuple[ParameterProvenance, ...]:
    """유산 역할과 닫힌 실패 닫힘 역할을 함께 담은 출처를 돌려준다.

    세 밀도값은 직렬화 호환을 위해서만 역사적 ``ce_prediction`` 문자열을 유지한다.
    존재비/작용 다리가 열려 있으므로 현재 :attr:`closure_role` 은
    ``legacy_model_boundary`` 다.
    """
    ...
    return (
        ParameterProvenance(
            ...
            params.omega_b0,
            "ce_prediction",
            "paper/검증_원장/상수_코어_원장.md §2.3 LEGACY_ROUNDED_RUNTIME_V1",
...
def dark_energy_scale(a: float, w0: float, wa: float) -> float:
    """CPL 척도 rho_de(a)/rho_de(1), w(a)=w0+wa(1-a) 다."""
    if a <= 0.0:
...
def residual_mu_of_a(a: float, params: CEForwardParams) -> float:
    """현상론적 성장 부문 잔차 결합이다. GR 은 정확히 mu=1 이다."""
...
def parse_bao_data(spec: str) -> tuple[BAODataPoint, ...]:
    """kind 가 {dm,dh,dv} 인 'z:kind:value:sigma,...' 를 파싱한다."""
...
def _validated_covariance_matrix(
    ...
    """오차 단위에 무관하게 유한성·대칭성·양의 정부호를 검사한다."""
    n = len(matrix)
    if any(len(row) != n for row in matrix):
        raise ValueError("covariance matrix must be square")
    if any(not math.isfinite(value) for row in matrix for value in row):
        raise ValueError("공분산의 모든 값은 유한해야 합니다")
    ...
    correlation = [
        [matrix[i][j] / scales[i] / scales[j] for j in range(n)]
        for i in range(n)
    ]
    for i in range(n):
        for j in range(i):
            if not math.isclose(
                correlation[i][j], correlation[j][i], rel_tol=1e-10, abs_tol=1e-12
            ):
                raise ValueError("covariance matrix must be symmetric")
            correlation[i][j] = correlation[j][i] = (
                correlation[i][j] / 2.0 + correlation[j][i] / 2.0
            )
    lower = [[0.0] * n for _ in range(n)]
    for i in range(n):
        for j in range(i + 1):
            residual = correlation[i][j] - math.fsum(
                lower[i][k] * lower[j][k] for k in range(j)
            )
            if not math.isfinite(residual) or (i == j and residual <= 0.0):
                raise ValueError("공분산은 양의 정부호여야 합니다")
            lower[i][j] = math.sqrt(residual) if i == j else residual / lower[j][j]
    return tuple(
        tuple(matrix[i][j] / 2.0 + matrix[j][i] / 2.0 for j in range(n))
        for i in range(n)
    )


def parse_covariance_matrix(spec: str) -> tuple[tuple[float, ...], ...]:
    """공분산 행을 파싱한다. 예: '0.04,0.01;0.01,0.09'."""
    text = spec.strip()
    if not text:
        return ()
    ...
    if not rows:
        return ()
    return _validated_covariance_matrix(tuple(rows))


def invert_matrix(matrix: tuple[tuple[float, ...], ...]) -> tuple[tuple[float, ...], ...]:
    """작은 조밀 행렬을 가우스-조던 소거로 역행렬 계산한다."""
    n = len(matrix)
    if n == 0:
        return ()
    if any(len(row) != n for row in matrix):
...
def regularized_gamma_q(shape: float, x: float) -> float:
    """정규화 상부 불완전 감마 함수 Q(shape, x) 를 돌려준다.

    x < shape + 1 이면 급수를, 아니면 연분수를 쓴다. 이 표준 분할로 SciPy 런타임
    의존을 피한다.
    """
    if shape <= 0.0:
...
def chi_square_verdict(p_value: float) -> str:
    """고정 모형 카이제곱 생존 확률을 감사 판정으로 사상한다.

    PASS 는 p >= 0.05 다. TENSION 은 0.0027 <= p < 0.05 다. REJECT 는 p < 0.0027 이며,
    관례적인 양측 가우스 3-시그마 꼬리를 사전등록된 심각 불일치 경계로 쓴다. 고정 모형의
    적합도 게이트이지 파라미터 추정이나 모형 선택 기준이 아니다.
    """
...
def assess_bao_fit(
    ...
    """압축 BAO 고정 모형 적합을 평가하고 전체 카이제곱을 분해한다.

    자유도는 N - fitted_parameter_count 다. 내장 CE 경계 검사에서는 13개 BAO 점에
    파라미터를 맞추지 않으므로 dof=13 이다. 관측별 공분산 기여는 ``r_i (C^-1 r)_i`` 로
    계산한다. 합은 정확히 ``r^T C^-1 r`` 이지만, 상관이 결합 불일치를 재배분하면 개별
    항은 음수일 수 있다.

    돌려주는 1-파라미터 척도 절제(ablation)는 예측 벡터 ``y`` 와 자료 벡터 ``d`` 에 대해
    ``q* = (y^T C^-1 d) / (y^T C^-1 y)`` 를 해석적으로 적합한다. 모든 BAO 거리비가
    ``1/(H0*rd)`` 로 비례하므로 외부 H0*rd 정규화에 대한 민감도를 진단한다.
    AIC=chi2+2k 와 BIC=chi2+k*log(N) 은 전체 적합 파라미터 수를 쓰므로 보고된 개선에
    척도 파라미터 벌점이 들어간다.
    진단 적합이지 CE 예측이나 승격된 모형 결과가 아니다.
    """
    if not data:
    ...
    covariance_used = _validated_covariance_matrix(covariance_used)
    predictions = tuple(
...
def bao_chi2_with_covariance(
    ...
    """주어진 공분산 행렬을 쓰는 전체 압축 BAO chi2 다."""
...
def solve_growth(
    ...
    """선형 성장 D 와 f=dlnD/dlna 를 풀고 D(a=1)=1 로 정규화한다."""
