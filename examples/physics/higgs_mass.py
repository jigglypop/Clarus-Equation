"""
CE 힉스 질량: M_H/M_Z = F = 1 + alpha_s*D_eff 의 유도
...
# M_H는 힉스 포텐셜의 곡률. "진동 스케일"
# M_H > M_Z인 이유: QCD 진공 보정이 힉스 자체 결합을 증강
#
# 구체적으로:
# 힉스 자체 결합 lambda_H는 bare 값에 QCD top-loop 보정을 받는다
# delta_lambda = -(3*y_t^4)/(8*pi^2) * ln(Lambda/M_t) + ...
# 그러나 CE에서는 running이 아니라 "폼팩터"를 사용
#
# 양성자 반경에서: F = 1 + alpha_s*D_eff (QCD 진공 증폭)
# 같은 F가 힉스-Z 비율에 적용된다면:
...
2. M_H = 힉스 포텐셜의 곡률
   "힉스 자체 결합 lambda_H를 통해 QCD에 민감"
   
3. QCD의 기여:
   top 쿼크 (y_t ~ 1)는 힉스 자체 결합에 가장 큰 루프 보정을 제공.
...
4. 따라서:
   ...
   "힉스 질량은 Z 질량에 QCD 진공(= 암흑물질 비율)만큼 증강된 것"
...
결론:
  ...
  힉스 질량은 "Z 질량에 QCD 진공 폼팩터를 곱한 것"이다.
