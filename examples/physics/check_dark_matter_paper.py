...
Input: geometric closure set (canonical branch)
  ...
  (2) alpha_total = 1/(2*pi)
  (3) 5-variable elimination root alpha_s = 0.11789
...
class Const:
    ...
    # Canonical branch root from 5-variable elimination
    alpha_s = 0.11789
    alpha_s_err = 0.00090

    # Electroweak quantities derived from alpha_s
    sin2_tW = 4.0 * alpha_s ** (4.0 / 3.0)
    sin2_tW_err = (16.0 / 3.0) * (alpha_s ** (1.0 / 3.0)) * alpha_s_err
    cos2_tW = 1.0 - sin2_tW
...
def part1_framework():
  ...
  A5: Dark sector energy splits into vacuum (DE) and fluctuations (DM):
        Omega_DM / Omega_Lambda = alpha_s * D_eff                    (3)
  ...
  | Input 2     | alpha_total          | 1/(2*pi)           | time-grid closure |
  | Derived     | alpha_s              | 0.11789            | 5-var elimination |
  | Derived     | sin^2(theta_W)       | {Const.sin2_tW:.5f}          | 4*alpha_s^(4/3) |
  ...
  | Derived     | alpha                | {Const.alpha_ratio:.5f}          | alpha_s * D_eff  |
  |-------------|----------------------|-------------------|-----------------|
  | FREE PARAMS | 0 (zero)             |                   |                 |
...
def part3_error_propagation():
    ...
    for key, label in [("Omega_b", "Omega_b"), ("Omega_DM", "Omega_DM"), ("Omega_Lambda", "Omega_Lambda")]:
        half = abs(pred_hi_als[key] - pred_lo_als[key]) / 2.0
        print(f"    {label:<10s}: {half:.2e}")
...
def part4_secondary_predictions():
    ...
    alpha_s_cosmo_err = r_err / pred["D_eff"]
    tension_as = abs(alpha_s_cosmo - Const.alpha_s) / math.sqrt(alpha_s_cosmo_err**2 + Const.alpha_s_err**2)

    print(f"    alpha_s(cosmo) = Omega_DM / (D_eff * Omega_Lambda)")
    print(f"                   = {alpha_s_cosmo:.5f} +/- {alpha_s_cosmo_err:.5f}")
    print(f"    alpha_s(closure) = {Const.alpha_s:.4f} +/- {Const.alpha_s_err:.4f}")
    ...
    print(f"    sin^2(theta_W)(closure)= {Const.sin2_tW:.5f} +/- {Const.sin2_tW_err:.5f}")
...
def part7_comprehensive_table():
    ...
    alpha_s_cosmo = ratio_obs / pred["D_eff"]
    alpha_s_cosmo_err = ratio_err / pred["D_eff"]
    ...
    delta_obs = D_eff_obs - 3.0
    disc = 1.0 - 4.0 * delta_obs
    sin2_cosmo = (1.0 - math.sqrt(disc)) / 2.0 if disc >= 0 else float("nan")
    ...
    sin2_cosmo_err = abs(s2_hi - s2_lo) / 2.0
    ratio_bDM_obs = pd["Omega_b"][0] / pd["Omega_DM"][0]
    ratio_bDM_err = ratio_bDM_obs * math.sqrt(
        (pd["Omega_b"][1] / pd["Omega_b"][0]) ** 2 +
        (pd["Omega_DM"][1] / pd["Omega_DM"][0]) ** 2
    )
    sin2_rel_pct = abs(sin2_cosmo - Const.sin2_tW) / Const.sin2_tW * 100.0
    alpha_reverse_sigma = abs(alpha_s_cosmo - Const.alpha_s) / math.sqrt(
        alpha_s_cosmo_err ** 2 + Const.alpha_s_err ** 2
    )
  ...
  +-----------+---------------------+-------------+-------------+----------+---------+""")
    ...
    rows = [
        ("Cosmo", "Omega_b", f"{pred['Omega_b']:.5f}", f"{pd['Omega_b'][0]:.5f}", f"{pd['Omega_b'][1]:.5f}", None),
        ("Cosmo", "Omega_Lambda", f"{pred['Omega_Lambda']:.4f}", f"{pd['Omega_Lambda'][0]:.4f}", f"{pd['Omega_Lambda'][1]:.4f}", None),
        ("Cosmo", "Omega_DM", f"{pred['Omega_DM']:.4f}", f"{pd['Omega_DM'][0]:.4f}", f"{pd['Omega_DM'][1]:.4f}", None),
        ("Cosmo", "DM/DE ratio", f"{pred['alpha']:.4f}", f"{ratio_obs:.4f}", f"{ratio_err:.4f}", None),
        ("Cross", "alpha_s(cosmo)", f"{alpha_s_cosmo:.4f}", f"{Const.alpha_s:.5f}", f"{alpha_s_cosmo_err:.4f}", None),
        ("Cross", "sin^2 tW(cosmo)", f"{sin2_cosmo:.4f}", f"{Const.sin2_tW:.5f}", f"{sin2_cosmo_err:.4f}", None),
        ("Cosmo", "Omega_b/Omega_DM", f"{pred['Omega_b']/pred['Omega_DM']:.4f}", f"{ratio_bDM_obs:.4f}", f"{ratio_bDM_err:.4f}", None),
    ...
    print(f"  sin^2(theta_W) reverse: {sin2_rel_pct:.2f}% off (limited by CMB Omega_b precision)")
    print(f"  alpha_s reverse: {alpha_reverse_sigma:.2f} sigma (most tense; test with DESI)")
...
def part8_dm_identity():
  ...
  Formal decomposition:
    ...
    + (1-eps^2)/(1+alpha_s*D_eff) (DE: vacuum energy of suppression field)
    + (1-eps^2)*alpha_s*D_eff/(1+alpha_s*D_eff) (DM: QCD-scale fluctuations)
