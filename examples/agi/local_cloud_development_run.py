"""One-shot hash-bound runner for the V10 local/cloud development phase."""
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: local_cloud_development_run.py <registration.json>")
    registration_path = Path(sys.argv[1]).resolve()
    registration = json.loads(registration_path.read_text(encoding="utf-8"))
    if registration.get("schema") != "clarus.local_cloud.development.v1":
    ...
    burned = set(registration["burned_diagnostic_seeds"])
    reserved = set(registration["reserved_confirmation_seeds"])
    if burned.intersection(seeds) or reserved.intersection(seeds):
        raise ValueError("development seeds overlap burned or reserved roles")
    output = (repository / registration["result_path"]).resolve()
    if output.exists():
        raise FileExistsError("registered development result already exists; rerun forbidden")
    ...
    payload = {
        "schema": "clarus.local_cloud.development.result.v1",
